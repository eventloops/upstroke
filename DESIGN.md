...
**v0.3+ — repository review attestation is a notary, not another reviewer.** A forge integration may turn Tactus's final integration verdict into an enforced pull-request requirement, but publication must never spend a second frontier review merely to tell the forge what Tactus already proved. Before attestation, the run produces exactly one run-level final integration verdict over the exact target-base-to-final-head diff and the resulting repository state; task or candidate verdicts, including merge verification, may be reused only when their recorded semantic scope is exactly that full range. A completed run exports an immutable attestation binding repository identity, exact base commit and tree, exact reviewed head commit and tree, review scope, run id, the frozen final-review policy, every required review pass in execution order (lens, agent, adapter, model, model family, pre-flight CLI version, effort, outcome, and evidence reference), the aggregate verdict derived fail-closed from that complete pass list, and an evidence digest. Open the draft PR early and let cheap forge CI run first. Publication is allowed only while the attested commit is still the PR head and the branch is current with the target; any push, rebase, conflict resolution, target advance, or integration change invalidates it and requires a final review of the new range. Forge-enforced mode requires repository-controlled workers, gates, and reviewers to run behind an external boundary with no access to coordinator state, evidence or attestation storage, or forge submission credentials; host-run mode remains local-only. A forge adapter uses the forge's official CLI to submit the attestation with authenticated provenance. A trusted default-branch workflow revalidates that provenance, repository/base/head/tree identity, review scope, frozen review policy, complete ordered pass list, derived aggregate verdict, and evidence digest and reruns repository-controlled gates in a secretless job; only after that succeeds may a separate protected publisher job obtain a dedicated forge App's signing key and publish the required exact-SHA check. The App is only a machine identity: it hosts no model, webhook, or review service. Rules bind the check name to that App's integration identity, so a same-named check from another source or SHA does not count. The forge-enforced identity is the full repository, target ref, reviewed base commit and tree, reviewed head commit and tree, and frozen review-policy tuple—not merely a check name and source commit. A forge adapter must prove that its merge rule enforces that same tuple. On GitHub, whose required-check predicate sees only context, App integration, and commit, enforcement is limited to one protected target per context, uses a distinct App-owned required-check context for each protected target, or binds the review to a provider-native merge-result commit; putting the pull request, base, or policy only in `external_id`, details, or evidence does not constrain the required check and is insufficient. GitHub is the first `ForgeAdapter`; another forge is supported only when it provides equivalent exact-commit status, protected publisher identity, trusted-execution capabilities, and merge-time enforcement of that full tuple, either by proving the reviewed base is still current for the uniquely bound target or by requiring the reviewed merge-result identity. Code implemented manually or outside Tactus may enter through a review-only Tactus run that produces the same final exact-head verdict and run-bound attestation, while repositories that do not want forge enforcement keep the local headless workflow unchanged. This protects the process from stale, spoofed, or self-minted PR output; it does not pretend to protect a repository from a dishonest administrator who controls its rules and secrets.
