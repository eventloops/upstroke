...
2. **Review** — a read-only worker profile receives task + acceptance + conventions brief + the complete engine-captured diff, and must end with a fenced JSON verdict (`pass`, `reasons`, `required_changes`). The engine parses the last fenced block; one re-ask on unparseable, then it counts as failure. Each pass has an independent wall-clock budget (90 minutes by default), and the re-ask shares that pass's deadline rather than starting a second clock. A diff above the complete-review limit refuses before reviewer spend and must be split; tactus never drops paths and calls the remainder reviewed. The reviewer prompt includes an anti-sycophancy instruction: its job is to find reasons to fail, not to agree.
