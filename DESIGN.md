...
**Effort is taken from the same run snapshot.** `run_started.effort_policy` records the resolved implementation value at small, mid, and frontier plus the review value. Every worker and every review pass reads that snapshot, so editing `[routing.effort]` between processes cannot make one run execute or judge at two standards. A mismatch warns and continues with the record; start a new run to adopt it. Logs predating the field remain schema-1 compatible: their first resume re-derives with an explicit warning and writes the established policy on `run_resumed`, after which later resumes are record-bound rather than repeatedly re-deriving.
