...
**v0.3+ — repository review attestation is a notary, not another reviewer.** A provider integration may turn Tactus's final integration verdict into an enforced pull-request requirement, but it must never spend a second frontier review merely to tell the forge what Tactus already proved. A completed run exports an immutable attestation binding repository identity, base, exact reviewed head commit and tree, run id, reviewer agent/model/family/effort, verdict, and an evidence digest; task-level passes are insufficient unless merge verification also produced a passing verdict for that final integrated head. Open the draft PR early and let cheap provider CI run first. Publication is allowed only while the attested commit is still the PR head and its branch is current with the target; any push, rebase, conflict resolution, or integration change invalidates it and requires review of the new final head. The provider adapter uses the provider's official CLI to submit the record, while a trusted default-branch workflow revalidates the exact SHA and evidence and reruns repository-controlled gates before a dedicated provider App publishes the required exact-SHA check. That App is only a machine identity: it hosts no model, webhook, or review service. Its signing key remains a provider-side protected secret unavailable to local Tactus runs and pull-request workflows, so the code being judged can never mint its own passing gate. Rules bind the check name to that App's integration identity; a same-named check from any other source or SHA does not count. GitHub is the first adapter, but the attestation and publisher seam remain provider-neutral for GitLab or other forges. Manual and non-Tactus contributions may enter the same exact-head review route, while repositories that do not want forge enforcement keep the local headless workflow unchanged. This protects the process from stale or spoofed PR output; it does not pretend to protect a repository from a dishonest administrator who controls its rules and secrets.
