...
- A **rustc** lint attribute is not so limited: any leg that compiles the region evaluates it, so a
  ...
  for every job, so today that is every leg — which means narrowing it would silently take the
  self-retirement guarantee with it.
