...
mod tests {
    ...
    ///
    /// That half is held by a **handshake rather than a timer**, and the
    /// difference is a measured failure rather than a preference. It first held
    /// the file for five seconds and read the margin over the retry budget — 39
    /// sleeps of 25ms, so under a second — as decisive. It is not. Those five
    /// seconds had to cover *everything* between the handle opening and the last
    /// retry attempt, and on a starved scheduler two separate stretches can eat
    /// them: the loop's own wall clock, since `STEP` bounds sleeping rather than
    /// elapsed time and every wake queues behind the runnable threads ahead of
    /// it; and the gap before removal even begins, since this thread must also
    /// be scheduled between `ready.recv()` and the call. Observed once on a
    /// 4-vCPU Windows guest at `--test-threads=16`: the holder released, a late
    /// attempt succeeded, and the control that exists to prove a held worktree
    /// is *reported* watched it get removed instead.
    ///
    ...
    /// captured from the original occurrence, so nothing here claims the loop
    /// itself outlived the timer. The handshake settles both, because it takes
    /// the wall clock out of the oracle entirely rather than budgeting it.
    ///
    /// A handshake alone would deadlock on the one regression this control is
    /// for, which is why `FAIL_SAFE` exists below: if the retry ever became
    /// unbounded, removal would never return, the sender would never drop, and
    /// the holder would wait behind a test that is itself waiting for removal —
    /// a hang to the CI job timeout in place of a failure. The timer version
    /// caught that case by accident; the fail-safe catches it on purpose, and
    /// reports it as the unbounded retry it is.
    #[cfg(windows)]
    #[test]
    fn a_worktree_whose_killed_child_is_still_closing_is_removed_not_refused() {
        ...
        /// unbounded retry is *reported here* rather than hanging until CI kills
        /// the job with no verdict at all.
        ///
        /// **Sized from measurement, because the first guess was wrong.** Two
        /// minutes looked like a hundred times the loop's nominal budget and it
        /// fired anyway, on a full-suite run of a loaded guest — which is
        /// `CODING_STANDARDS.md` §12's own warning, that a bound short enough to
        /// expire on a loaded runner has become the signal itself. Measured
        /// instead, with the whole suite running: the locked case's
        /// `remove_worktree` returns in **1.0s** unpinned and **6.5–7.6s** pinned
        /// to 4 CPUs at `--test-threads=16`. Ten minutes is some seventy-five
        /// times the worst of those, and still well inside the CI job timeout, so
        /// it bounds a loop that has *wedged* rather than one that is merely slow.
        ...
        ///
        /// Dropping a `JoinHandle` only detaches its thread. Without this guard a
        /// failing assertion left `Fixture::drop` — a single best-effort
        ...
        impl Drop for Holder {
            fn drop(&mut self) {
                // Disconnect before joining, always: the holder blocks on this
                // sender, so joining first is the deadlock.
        ...
        // `Some(d)`: a process whose last handle closes `d` after the removal
        // starts, which is the window the retry exists to cross. `None`: one that
        ...
        for (hold, expected_removal) in [
            ...
            // last handle is still closing. The closing case releases it on a
            // timer; the held case releases it only when `Holder` is dropped.
            ...
            let handle = std::thread::spawn(move || {
                ...
                match hold {
                    ...
                    // Held until `Holder` drops the sender, which happens only
                    // once the assertions have run -- and during unwinding, so a
                    // failing assertion releases the handle rather than leaving
                    // the tree locked against the fixture's own cleanup. A timer
                    // here could only ever prove the loop is shorter than that
                    // timer, which is the assumption that made this test a flake.
                    //
                    // `Disconnected` is that release. `Timeout` is the fail-safe,
                    // and reaching it means removal never returned -- recorded
                    // rather than acted on, so the assertion below can name the
                    // unbounded retry instead of whatever the freed handle then
                    // allowed to succeed.
                    None => {
                        if let Err(mpsc::RecvTimeoutError::Timeout) =
                            released.recv_timeout(FAIL_SAFE)
                        {
            ...
            // After `fixture`, so it drops before it. Nothing below may `join`
            // this thread by hand: the guard owns both halves precisely so the
            // disconnect-then-join order holds on every path out of the loop.
            let _holder = Holder {
            ...
            // exists to catch -- and without this the `Ok` that the freed handle
            // then permitted would be reported in its place. The elapsed time is
            // quoted because it is the number that says whether the budget or the
            // fail-safe is the thing that needs changing.
            assert!(
            ...
            if expected_removal {
                ...
                // while the handle was held on a five-second timer. That message
                // is the recorded fingerprint of a retired flake, and a failure
                // here now means removal succeeded against a handle that was
                // provably *still open* -- a defect, not a scheduling artifact.
                // Reusing the text would invite the next reader to discount it.
                let error = outcome.expect_err(
                    "a handle still held when removal returns is a locked worktree, not a closing \
                     one, and must be reported rather than waited on forever",
                );
                assert!(
                    target.exists(),
                ...
                // The *reason*, not merely the fact of a failure. The loop
                // returns its last attempt's error, and only a sharing violation
                // or a delete-pending denial says it spent its budget against
                // this handle. Without this the control is satisfied by any
                // error at all, including a containment refusal raised before
                // removal is ever attempted -- which would prove nothing about
                ...
                // -- an unreadable `.git/worktrees` answers error 5 too -- before
                // the funnel is entered, leaving the target on disk, so both
                // assertions above would hold without the loop ever running. Only
                ...
                assert!(
                    ...
                    "the refusal must name the worktree the loop failed to remove, so that it is \
                     the budget being reported and not an earlier step: {}",
                    refusal_of(&error)
                );
            }
        }
