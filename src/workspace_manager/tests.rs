...
/// deleted the victim's file with every check passed. The same seam for the
/// write direction is the test below.
#[test]
fn an_intents_directory_exchanged_at_the_before_hook_refuses_the_intent_removal() {
    ...
    let mut hooks = ExchangeIntentsAtBefore {
        site: slot.remove_intent_site(),
        intents: fixture.manager.execution_root().join("intents"),
...
/// The write direction of the test above: `write_intent` stages and renames
/// inside `intents/`, so an exchanged `intents/` would put the record, and
/// its staging file, in the victim directory.
#[test]
...
/// A `Before` hook that exchanges the execution root's `intents/` directory
/// for a link to `victim`, at one site.
struct ExchangeIntentsAtBefore {
    site: EffectSiteId,
    intents: PathBuf,
    moved: PathBuf,
    victim: PathBuf,
...
impl EffectHooks for ExchangeIntentsAtBefore {
    fn phase(&mut self, site: EffectSiteId, phase: HookPhase) -> Injection {
        if site == self.site && phase == HookPhase::Before {
            fs::rename(&self.intents, &self.moved).expect("move the real intents directory aside");
            plant_directory_link(&self.victim, &self.intents);
        }
        Injection::Proceed
    }
}
