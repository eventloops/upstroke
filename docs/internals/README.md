...
byte-string, C-string, raw-string and character literals, and every other code
byte, before it looks for the phrase, so an ordinary literal that happens to
contain `Extended notes:` is source rather than a marker, and "its first line of
code" is the first line carrying anything outside a comment. The scan carries
comment kind as well: a mention inside a `/* ... */` block counts toward the
one-marker rule and is refused however its body is spelled, because the marker
is a `//` line comment and no other form.
...
files, misplaced or duplicate markers, and the literals and block comments a
raw-text scan reads as markers.
