...
## `pub enum TaskRunStatus` › `Unknown,`
...
## `pub struct TaskReport` › `pub model: String,`
...
## `pub struct TaskReport` › `pub review_models: Vec<String>,`
...
## `impl TaskReport` › `pub fn trail(&self) -> String {`
...
## `pub struct RunReport` › `pub gates: Vec<String>,`

Effective gate names, and whether they came from config or derivation.

## `pub struct RunReport` › `pub halted_at: Option<String>,`

Task id the run halted at, if any.

## `pub struct RunReport` › `pub questions: Vec<QuestionRecord>,`

Every question raised, with its answer where one arrived (§12).

## `pub struct RunReport` › `pub budget_stop: Option<events::BudgetExceeded>,`

The §13 ceiling that stopped the run, if one did.

## `pub struct RunReport` › `pub pool_drain: Vec<PoolDrainRow>,`

What each pool drained, folded from this run's own attempts (§13).

## `pub struct RunReport` › `pub running: bool,`
...
## `pub struct RunReport` › `pub interrupted: bool,`
...
## `impl RunReport` › `fn committed_count(&self) -> usize {`
...
## `impl RunReport` › `pub fn outcome(&self) -> RunOutcome {`
...
## `impl RunReport` › `pub fn from_state(`
...
## `pub(super) struct ReportHeader<'a>` › `pub(super) running: bool,`

Whether an engine is driving this run right now.

## `pub(super) struct ReportHeader<'a>` › `pub(super) interrupted: bool,`
...
## `pub fn topo_order(plan: &Plan) -> Vec<usize>` › `for (i, flag) in done.iter_mut().enumerate() {`

Unreachable on a validated plan; degrade to plan order.

## `pub fn render(&self) -> String` › `let partial = if task.review_cost_incomplete { "?" } else { "" };`
...
## `pub fn render(&self) -> String` › `(models, None) => format!(" + review {} $?", models.join(", ")),`

Reviewed only by routes that report no spend (§13) —
say who judged it rather than imply it was free.

## `pub fn render(&self) -> String` › `let worker = match task.cost_usd {`
...
## `pub fn render(&self) -> String` › `let ending = if self.interrupted {`
...
## `pub fn render(&self) -> String` › `TaskRunStatus::Unknown => {`
...
## `pub fn render(&self) -> String` › `let stopped = self.budget_stop.as_ref().map_or_else(`
...
## `impl RunReport` › `pub fn render_ledger(&self) -> String {`
...
## `pub fn render_ledger(&self) -> String` › `let partial = |rendered: String, incomplete: bool| {`
...
## `pub fn render_ledger(&self) -> String` › `if self.pool_drain.is_empty() {`
...
## `pub fn render_ledger(&self) -> String` › `None => "— (this route reports no spend)".to_owned(),`
