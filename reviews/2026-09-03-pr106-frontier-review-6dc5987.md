...
| field | value |
|---|---|
| **Binding verdict** | **CHANGES_REQUIRED** at `6dc5987`. **This record does NOT bind to the head this pull request merges.** A `CHANGES_REQUIRED` pass is followed by a repair, so the current head is later than the reviewed SHA below; see the delta note for what changed and whether a further pass exists |
| **Reviewed SHA** | `6dc5987ff926dc226d3c9802b1f744cee2465750` |
| Passes | 5 , each recorded below as it was returned |
...
This change went round 5 times. Every pass is recorded, in order, with
...
seen the tree at the current head.** The reviewed SHA is `6dc5987`; any
