# PR #104 — refactor: remove the runtime fixture reads outside validate.rs: frontier review record
...
>
> VERDICT: CHANGES_REQUIRED
## Pass 9 — `4ba4149` — CHANGES_REQUIRED

Reviewed SHA `4ba41499cb2537215cb19d3c43260fc094d0fa81`. Blockquoted byte-for-byte from the reviewer's
saved output (`~/review-pr104-4ba4149.md` on the review box).

> ## Findings
>
> 1. **P3 — The review evidence is not bound to the exact head.** `reviews/2026-09-03-pr104-frontier-review-ae59f2d.md:5-6` calls `ae59f2d` “the head this pull request merges,” but HEAD is `4ba4149`; the intervening delta materially changes `Cargo.toml`, `fixtures/`, `src/validate.rs`, and `src/plan/mod.rs`. `pr.md:1052-1055` admits pass 9 is pending, while `pr.md:1066-1069` names a durable record that no longer exists and `pr.md:1090-1106` still says the PR is parked at pass 6. An auditor following the claimed durable URL gets no file; following the new record gets a verdict for the wrong source tree. Likewise, the recorded eight-command baseline is at `c630681`, despite `pr.md:904-905` calling it the “exact pushed head.” This does not satisfy the build-box rule to record the passing head.
>
> 2. **P3 — The documented full rollback omits two PR commits.** `pr.md:42` and `pr.md:1141-1149` say there are 18 commits and that the listed command reverts every one. There are 20 commits after the base; the command omits `b84205a` and `4ba4149`. Following it leaves `reviews/2026-09-03-pr104-frontier-review-ae59f2d.md` added relative to master, so it does not restore the base tree. This also makes ledger row `PR104-R1-004`’s `fixed` claim false again.
>
> 3. **P3 — The finding ledger’s prevention evidence contradicts the final tree.** Examples:
>
>    - `pr.md:1159` says deleted `Corpus` tests and `fs::create_dir` prevent the leak “at the head”; none exists at this head.
>    - `pr.md:1160` claims four constants and no fixtures directory; the head has three constants and all four fixture files.
>    - `pr.md:1162` claims the rollback contains every SHA; it does not.
>    - `pr.md:1190-1197` says every row is historical and prevented now because the code is gone, contradicting the preceding rows.
>
>    Concrete failure: the syntactic body and ledger validators pass, then an owner relies on these `fixed` dispositions when checking the final attestation even though their stated evidence describes an intermediate tree or its opposite.
>
> The implementation itself does achieve the narrow functional claim: the supplied patch exactly matches `1cbdccd..4ba4149`; the fixture blobs and `validate.rs` match master; searches found no remaining plan-fixture filesystem read outside `validate.rs`; and the affected markdown and registry tests passed even when the compiled binary ran from a directory without `fixtures/`. I found no new production panic, `anyhow`, native-path, Windows, DESIGN.md, or immutable-decision violation. The Scope statement at `pr.md:52` nevertheless incorrectly calls the three test files the “whole footprint”; the 214-line review record is a fourth, non-test file, although it is disclosed later rather than silently hidden.
>
> ## Out of scope
>
> - `src/validate.rs:367-407` retains PID-derived directories, `create_dir_all` adoption, unowned pre-deletion, and no cleanup. The file is byte-identical to the base and this final diff neither introduces nor activates those operations. After PID reuse, `scratch_root` can delete a foreign tree, or `opts` can adopt stale contents.
> - `src/validate.rs:632-824` retains relative `fixtures/*.md` paths. These sites are unchanged from base and no new caller is routed through them. Running the compiled validation tests from a directory without `fixtures/` makes the reads fail before their validation assertions.
>
> VERDICT: CHANGES_REQUIRED
