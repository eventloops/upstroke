...
| **An item inserted into a file re-targeting the doc comment above it** | **2** | both in PR7's session: two documented fold readers inserted above `questions_open` took its doc block (`bb68cf6`); the `ReviewPasses` trait inserted into the middle of `AttemptPlan`'s doc block, splitting one sentence across two items and shipping that way at the green head `9fcca67` | **No gate in this repository catches it.** `cargo doc` renders the result without complaint, `rustdoc::broken_intra_doc_links` sees valid links, clippy sees valid code, and the suite is green — the damage is that a type's documented rationale now describes a different type, which is a false claim about the code and the class this project treats as a defect rather than as cosmetics. Both occurrences came from **line-arithmetic insertion** (`sed`/`python` by index) rather than from editing in place, which is what makes it a method signal: the inserted item lands between a doc block and the item that block belongs to, and nothing in the byte stream marks that boundary. **The rule: after inserting any item into an existing file, read the doc block immediately above the insertion point and the one immediately below it, and check each still sits on the item it describes.** A doc block whose last sentence does not terminate is the tell |
...
| **A fold field's witness proves the accumulation and not the read** | **2** | `PR7-FOLD-DEFERS-ACCUMULATOR` (the fold-level replay witness was green while replacing `TopologyRun::deferrals_recorded` with a constant zero left the whole suite green); `PR7-FOLD-LADDER-POSITION` (same shape, same day: the escalation-arm mutation died instantly, the `ladder_position` mutation survived) | **The two halves are different claims and only one of them was ever being tested.** A fold witness asserts *the field is derived by replay and survives a resume*; it cannot see the driver ignoring the field, because the driver is not in it. Both times the accumulation was witnessed the same day the read shipped unwitnessed, and both times the read was the half that mattered — a correct field nobody consumes is a correct field nobody consumes. It is worse than an ordinary coverage gap because the fold witness *looks* like coverage of the feature and is cited as such. **The rule: every fold field a driver consumes carries two witnesses — one that the fold derives it by replay, and one that the driver's behaviour changes when the field does.** The second is written by mutating the driver's reader to a constant and requiring a named test to fail; a fixture that cannot make the field observable (the deferral count needed a *prior* deferral in the log; the ladder position needed a *spent* allowance) is a fixture that has not tested the read at all |
