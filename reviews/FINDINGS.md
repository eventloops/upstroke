...
and `quiescence` reads a `common_git_dir` failure as `Missing`. They are deferred to the
parent's sweep, the queue's last row of this family, because the helpers are where the repair
lives and the brief confines this pull request to the child; `quiet_object_lookup`, which PR #120
added to the parent for the manager's own lookups, is the shape that repair takes.
