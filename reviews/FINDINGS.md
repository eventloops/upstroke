...
| PR130-SNAPREF-OBJECTID-BELONGS-BESIDE-ITS-PREDICATES | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager/object.rs:62 | `ObjectId` lives in `snapshot_ref.rs` because that module is its only consumer, while `is_object_id` and the ref refusals take `&str` and the engine's `CommitSha` validates nothing -> one crate has two spellings of "an object id", a type and a predicate -> a ref primitive still accepts, then refuses at its own site, a value a snapshot input cannot be built from, and the two refusals (`MalformedObjectId`, `NotAnObjectId`) describe one fact | pre_existing | correctness | 7a83e69 | deferred to the parent's sweep of `src/workspace_manager.rs` (`standards/SWEEP.md` queue row 11), where the primitives that would take the type live: move `ObjectId` beside its predicates in `object.rs`, take it on both sides of `create_ref_zero_old`, `compare_and_swap_ref` and `delete_ref_expected_old`, and let the engine's `CommitSha` carry one; until then `ObjectId::new` is the one validated entry and `a_malformed_value_is_refused_as_not_an_object_id_with_the_value_as_offered` pins it | deferred |
...
| PR128-RESIDUE-RECORD-FOR-FOLDS-A-FAILED-WORKTREE-LIST | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager.rs:3063 | `record_for` answers `None` when `git worktree list --porcelain -z` exits non-zero -> the classifier's three add sites read `None` as not registered, so the after phase is absent and `RegisteredUnpopulatedWorktree` is absent -> `classify_object_residue` answers `None` for a repository Git could not open, the class no action recovers; a spawn failure is propagated, a failure that speaks is not; no production caller reaches the add sites today | pre_existing | correctness | 7a83e69 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-RESIDUE-OBJECT-EXISTS-FOLDS-A-FAILED-CAT-FILE | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager.rs:3003 | `object_exists` reads any non-zero `cat-file -e` as no such object, while `WorkspaceManager::object_exists` promises "a Git error other than no such object" -> the two commit-tree sites' after phase and `CandidateWriteTree`'s read `false` -> `verify_object` in `src/engine/topology/candidate.rs` refuses `ObjectMissing` for a repository Git could not open, naming a lost candidate where the failure was Git's; the one production path through the classifier today | pre_existing | correctness | 7a83e69 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-RESIDUE-HEAD-COMMIT-FOLDS-A-FAILED-REV-PARSE | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager.rs:3008 | `head_commit` answers `None` for any non-zero `rev-parse --verify --quiet HEAD` -> `ProposalCherryPick`'s after phase reads a Git failure as no HEAD -> the site classifies `None` or `Internal` for a worktree whose state is unknown; no production caller reaches the site today | pre_existing | correctness | 7a83e69 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-RESIDUE-INDEX-DIFFERS-READS-A-FAILED-DIFF-AS-THE-AFTER-PHASE | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager.rs:3051 | `index_differs_from_head` reads any non-zero `diff --cached --quiet` as differs, where exit 1 is the answer and 128 is a failure -> at `RepairMaterialize` a Git failure is the after phase -> `classify_object_residue` answers `After` for a worktree it could not inspect, the fold in the other direction: a failure read as success; no production caller reaches the site today | pre_existing | correctness | 7a83e69 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-RESIDUE-UNREACHABLE-OBJECTS-IGNORES-THE-EXIT-STATUS | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager.rs:2924 | `unreachable_objects` parses stdout and never reads the exit status of `git fsck` -> a fsck that failed lists nothing -> `UnreferencedObject` is never observed at any site, and `CandidateWriteTree`'s after phase reads a present tree as reachable; a failure read as no residue at every Object site; no production caller reaches these arms today | pre_existing | correctness | 7a83e69 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-RESIDUE-QUIESCENCE-READS-A-COMMON-DIR-FAILURE-AS-MISSING | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager.rs:1613 | `quiescence` matches `common_git_dir`'s error as `VerifyFailure::Missing` -> a linked worktree whose git dir the process cannot search is reported missing, and `administrative_residue_at`'s error is never reached through the public method -> the child's new error is observable only by driving the helper directly, which its witness does; a caller acting on `Missing` treats a worktree it could not read as one that is not there | pre_existing | correctness | 7a83e69 | deferred to the parent's sweep of `src/workspace_manager.rs`, the queue's last row of this family; `a_git_dir_that_cannot_be_searched_is_an_error_and_not_an_absent_residue` says in its doc why it drives `administrative_residue_at` directly | deferred |
...
| PR128-REVIEW-UNREADABLE-PACK-READS-AS-ABSENT-AND-AS-DIFFERING | P2 | b5c12e042b01798d7f0dc8bec15543da9d807a33 / src/workspace_manager.rs:2935 | the lookup helper took exit 1 with empty stderr as absence and `index_differs_from_head` took exit 1 as a difference -> with a pack file at mode 000 and its `.idx` readable, git 2.43 answers `rev-parse --verify --quiet <id>^{}` with exit 1 in silence and `diff --cached --quiet` with exit 1, and `cat-file --batch-check` prints `missing` (measured) -> `WorkspaceManager::object_exists` answered `false` and `refuse_absent_source` refused a missing candidate for a store it could not read, and `RepairMaterialize` classified a clean but unreadable repository `After`; the pull request's claim that an unreadable store propagates an error held only for a store Git itself refuses to open | introduced_by_feature | correctness | 335bb27 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, with the whole parent-helper programme, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-REVIEW-ADD-STATE-DOES-NOT-ESTABLISH-A-GIT-DIRECTORY | P2 | b5c12e042b01798d7f0dc8bec15543da9d807a33 / src/workspace_manager/residue.rs:451 | `git_dir_of` returned `Some(PathBuf::new())` for a pointer reading `gitdir:` with an empty path and `Some` for any target without reading it -> `add_state` answered `Populated` for a registered worktree with no git directory behind its pointer, the state the pull request claimed to fix, and every later name was joined onto `""` -> the add sites classified `After`; git 2.43 refuses the empty pointer as "invalid gitfile format" | introduced_by_feature | correctness | a17b8c5 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, with the whole parent-helper programme, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-REVIEW-RECORD-FOR-READS-AN-UNLISTABLE-REGISTRATION-AS-ABSENT | P2 | b5c12e042b01798d7f0dc8bec15543da9d807a33 / src/workspace_manager.rs:3108 | `record_for` trusted a listing that exited 0 -> with a registration's administrative directory at mode 000, `git worktree list --porcelain -z` exits 0 with nothing on stderr and omits it (measured on git 2.43) -> `add_state` answered `Unregistered` and the add sites classified `None` for a registration the process could not read; exit status alone did not make the listing complete | introduced_by_feature | correctness | 335bb27 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, with the whole parent-helper programme, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-REVIEW-GIT-FAILURES-CARRY-NO-COMMAND-OR-STDOUT | P2 | b5c12e042b01798d7f0dc8bec15543da9d807a33 / src/workspace_manager.rs:2891 | `read_only_git`'s spawn failure said only "failed to run git", and `read_only_git_failure` carried stderr alone -> a corrupt repository makes `fsck` exit 2 with `missing blob <id>` on stdout and nothing on stderr (measured on git 2.43), so `unreachable_objects` returned an error with a blank diagnosis, and a spawn failure named neither command nor directory -> the §7 operation-context claim the body made for every propagated `?` was false at the source | introduced_by_feature | correctness | 335bb27 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-REVIEW-INSPECTION-BEHAVIOUR-HAS-NO-DESIGN-SENTENCE | P2 | b5c12e042b01798d7f0dc8bec15543da9d807a33 / DESIGN.md:10 | `object_exists` and `unreachable_objects` changed public behaviour with no `design/` sentence -> the sole-authority rule and §13's same-change requirement were not met -> a reviewer had no living authority to check the behaviour against | introduced_by_feature | docs-contract | 335bb27 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-REVIEW2-REGISTRATION-GITDIR-STATTED-NOT-READ | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:3249 | the registration scan called `symlink_metadata` on each `gitdir` and never read it -> a `gitdir` at mode 000 stats fine, and `git worktree list` exits 0 and omits that worktree (measured on git 2.43), as does an empty `gitdir` -> `record_for` answered `None`, `add_state` `Unregistered` and the classifier `ObjectResidue::None` for a registration this process could not read; the pass-1 test chmod-ed the containing directory, so it missed the case | introduced_by_feature | correctness | 87c29fc | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-REVIEW2-STORE-SCAN-CLAIMS-MORE-THAN-IT-READS | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:3004 | the object-store scan read only the directory `--git-path objects` names and ignored `objects/info/alternates` -> in a `git clone --shared`, making the alternate's pack unreadable left the primary scan passing -> `object_exists` answered `false` while the design sentence the same repair added said absence is taken only from a readable store: the guarantee was false wherever a store is shared | introduced_by_feature | correctness | 87c29fc | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-REVIEW2-STORE-SCAN-HANGS-ON-A-FIFO | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:3037 | the scan opened every name ending in `.pack` or `.idx` without checking its file type -> a FIFO named `objects/pack/ignored.pack`, which git ignores, blocks `File::open` until a writer appears -> a residue classification, or any `object_exists`, hangs for ever, with no timeout or cancellation anywhere on the path; reproduced by the reviewer | introduced_by_feature | liveness | 87c29fc | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-STORE-READABILITY-NOT-ESTABLISHED | P3 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:3004 | git reports an object it cannot read as one it does not have: with a pack unreadable, `rev-parse --verify --quiet <id>` with a peel exits 1 in silence, exactly as for a missing object (measured on git 2.43) -> nothing at this layer distinguishes a storage failure from absence -> `object_exists` answers `false`, and `refuse_absent_source` refuses a dispatch naming a lost candidate, for a repository whose store is unreadable; latent, since a store the engine cannot read fails at its next write anyway | pre_existing | correctness | 7a83e69 | deferred, with the proposal: proving a store readable means reading it the way git does — `objects/info/alternates` followed to each alternate, a file-type check before every open, and a bound on every open so a FIFO cannot block — which belongs to whoever owns the object store in `src/workspace_manager.rs`, not to a residue classifier; this pull request's first two attempts at it produced the rows above, and the whole programme is reverted at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555, so nothing in this pull request claims a store is readable and the parent's `object_exists` answers exactly as it does on master | deferred |
| PR128-REVIEW2-PATHS-READ-AS-UTF8 | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:2868 | `git_dir_of` read the `.git` pointer with `read_to_string`, and `object_directory` and `common_git_dir` converted git's printed path with `String::from_utf8` -> in a valid repository whose path contains byte `0xff`, git writes and accepts that byte in a linked worktree's pointer and prints it back -> the add classifier answered `Io` where it had answered `After`, and every residue element read through the objects directory failed, against §8 in code this pull request rewrote | introduced_by_feature | portability | a17b8c5 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-REVIEW2-SYMLINKED-GIT-DIR-RECLASSIFIED | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:2904 | `git_dir_of` checked the pointer's target with `symlink_metadata(..).is_dir()`, which is false for a symlink to a directory -> an administrative directory moved and symlinked at its old name is one git reads (`git status`, `rev-parse --git-dir` and `worktree list` all work) -> a populated worktree classified `Internal` with `RegisteredUnpopulatedWorktree` observed, a reclassification the body did not disclose | introduced_by_feature | correctness | a17b8c5 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-REVIEW2-ERROR-CONTRACTS-FALSE | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:2362 | `WorkspaceManager::object_exists` documented a Git error while the store scan made it return `UpstrokeError::Io`, which the pull request's own test asserted, and `classify_object_residue` documented I/O only for residue names while the same scan returned it for object-store paths -> a caller matching on the documented variant misses the one it gets -> §13, in a contract this pull request had just rewritten | introduced_by_feature | docs-contract | 87c29fc | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-REVIEW2-DESIGN-PARAGRAPH-SPLITS-THE-TABLE | P2 | f161c9e8555c88474f09f89f095737346070d334 / design/26_design_merge_queue_protocol.md:304 | the added paragraph was placed before the resume table's last row -> the blank line terminated the table -> the `task_merged` row rendered as orphaned pipe text, so the change altered the structure of the sole design authority rather than adding to it | introduced_by_feature | docs-contract | 87c29fc | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-COMMONDIR-UNWRITTEN-READ-AS-AN-INSPECTION-FAILURE | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager/residue.rs:464 | `git worktree add` creates the administrative directory and writes `commondir` after, so a kill in between leaves a zero-length one -> git 2.43 then refuses to enumerate any worktree ("failed to read `…/commondir`", exit 128), which since PR #127's `record_for` repair is an error -> `add_state` propagated it and the classifier answered no class for durable state the site registers; the sampling harness counted it unclassified in 2 of 26 box runs, always `Worktree.Add: n=8 none=1 internal=6 after=0 unclassified=1` | fix_regression | correctness | PR127-RECORD-FOR-COLLAPSES-A-GIT-FAILURE-INTO-ABSENCE | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-NON-UTF8-TEST-CANNOT-RUN-ON-MACOS | P2 | f5b75d7a8088c801f7efa599ee1afe04c0ba6eb9 / src/workspace_manager/tests.rs:6810 | the §8 witness built its fixture with a `0xff` byte in the directory name under `#[cfg(unix)]` -> macOS rejects a filename that is not valid UTF-8 at creation, APFS answering `EILSEQ` -> CI's `test (macos-latest)` leg failed at `f5b75d7` ("a repository directory whose name is not UTF-8: Os { code: 92, kind: Uncategorized, message: \"Illegal byte sequence\" }", 1892 passed, 1 failed), a defect in this pull request's own test and not in the code it guards, since every other job was green | introduced_by_feature | portability | 55ed9d3 | the repair is **reverted** at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 on the coordinator's final instruction of 2026-09-04 15:35Z, after pass 3 found the same defect classes in the machinery each round added: the parent helper is master's again and this row is deferred to the sweep of `src/workspace_manager.rs`, the queue's last row of this family, with the reproduction it carries. Making an inspection's absence trustworthy means reading a repository the way Git reads it, which is that file's work; a design sentence follows that code rather than preceding it | deferred |
| PR128-PARENT-DANGLING-GITDIR-SYMLINK-READS-AS-ABSENT | P2 | dfc238c63fd7db4c9f9d8ab5f41113ad8ad56617 / src/workspace_manager.rs:3292 | a registration's `gitdir` replaced by a dangling symlink is `NotFound` when followed, though the name exists -> `git worktree list` succeeds and omits that entry, and any scan that treats `NotFound` as "no registration here" agrees with the omission -> `record_for` answers `None`, `add_state` `Unregistered` and the classifier `ObjectResidue::None` for a registration that is there; the same holds for a dangling `commondir`, which reads as an unfinished add rather than an inspection failure. Git's own omission follows its linked-worktree reader returning no worktree when `gitdir` cannot be read (git 2.43 `worktree.c`) | pre_existing | correctness | 7a83e69 | deferred to the sweep of `src/workspace_manager.rs`, queue row 11, with the reproduction: replace a linked worktree registration's `gitdir` with a dangling symlink, list, classify. A reader that distinguishes "the name is not there" from "the name is there and unreadable" needs `symlink_metadata` before the read and a decision for each kind, which is the repository-reading helpers' work, not a classifier's | deferred |
| PR128-PARENT-GIT-TRACE-MAKES-ABSENCE-AN-ERROR | P2 | dfc238c63fd7db4c9f9d8ab5f41113ad8ad56617 / src/workspace_manager.rs:3035 | `read_only_git` inherits the environment, so `GIT_TRACE=1` puts trace text on stderr for every command -> a lookup that recognises absence only as "exit 1 with both streams empty" stops recognising it -> `object_exists` returns a Git error instead of `false` under standard Git tracing, and the engine's missing-object refusal is never produced; an unborn `HEAD` fails the same way | pre_existing | correctness | 7a83e69 | deferred to the sweep of `src/workspace_manager.rs`, queue row 11, with the reproduction: run with `GIT_TRACE=1` and query a missing object. The repair is for the helper that runs Git: either a trace-proof environment for inspections, or an absence test that does not depend on stderr being empty; both are decisions about how this project runs Git, which is the parent's | deferred |
| PR128-PARENT-DIRECT-REF-TARGET-AND-QUIESCENCE-STILL-FOLD | P2 | dfc238c63fd7db4c9f9d8ab5f41113ad8ad56617 / src/workspace_manager.rs:2019 | `direct_ref_target` maps every unsuccessful `git show-ref --verify` to `None`, including a repository failure, and candidate recovery decides on that answer; `quiescence` maps a `common_git_dir` failure to `VerifyFailure::Missing` -> two inspection paths outside this file's reach fold failure into an answer -> a design sentence that says every repository read propagates its failure is false while they do, which is why this pull request's design paragraph is reverted rather than narrowed | pre_existing | correctness | 7a83e69 | deferred to the sweep of `src/workspace_manager.rs`, queue row 11: the two helpers and the design sentence belong to the same change, and the sentence follows the code. Until then no living authority claims more than the code does | deferred |
| PR128-PARENT-GITFILE-GRAMMAR-NOT-GITS | P2 | dfc238c63fd7db4c9f9d8ab5f41113ad8ad56617 / src/workspace_manager.rs:2872 | `git_dir_of` accepts `gitdir:` without the space Git requires, trims leading and trailing blanks Git does not, and takes any directory as a git directory -> a checkout whose `.git` reads `gitdir:/path/to/admin`, or points at an ordinary empty directory, is rejected by Git and accepted here -> `add_state` answers `Populated` and the classifier `After` for a worktree Git will not open, which is the opposite of the interrupted-add residue the site registers | pre_existing | correctness | 7a83e69 | deferred to the sweep of `src/workspace_manager.rs`, queue row 11, with the reproduction: rewrite a valid checkout's `.git` to `gitdir:/path/to/admin`, and separately point a well-formed line at an empty directory. Reading the gitfile as Git reads it (`setup.c`, git 2.43) and validating the target as a git directory is that file's work; `add_state` reads whatever it answers | deferred |
| PR128-PARENT-UNBOUNDED-READS-OF-REPOSITORY-CONTROLLED-FILES | P2 | dfc238c63fd7db4c9f9d8ab5f41113ad8ad56617 / src/workspace_manager.rs:3319 | the registration scan reads `gitdir` and `commondir` with unrestricted `fs::read` -> a FIFO at `.git/worktrees/poison/gitdir` blocks the open for ever with no writer, and a symlink to `/dev/zero` reads without bound -> classifying an add site hangs or exhausts memory; bounding the number of registrations bounds neither bytes nor time, so the Risk claim that no inspection can block on a FIFO was false | introduced_by_feature | liveness | 87c29fc | fixed at 59f2bd99c95c80f1a2a011a78ebe34b43fbf4555 by reverting the scan, so no production path in this pull request reads a repository-controlled file; deferred as a requirement to the sweep of `src/workspace_manager.rs`, queue row 11: every read of a file a repository controls needs a file-type check before the open and a bound on bytes and on time, and that requirement covers `revalidate_removal`'s existing `gitdir` reads as much as anything new | deferred |
...
## 55. PR #135 fixture.rs sweep (2026-09-04)

Append-only. The sweep of `src/workspace_manager/fixture.rs` (PR #135), row 9 of the review
queue, under the owner's amendment 7 of 2026-09-04: correctness, a better implementation, the
standards, and the tests, in that order. These are the rows of the sweeping session's own
line-by-line review at the base `95c5bd3`, recorded before any frontier pass; the passes the
coordinator launches append their rows below these. `origin/master` was merged in twice on the
coordinator's instruction of 2026-09-04, at `c61880f` after #131 landed and at `5f661fa` after
#109 landed, so the locations below are the base the review was done at and the head is downstream
of all three. #109 touches this file's subject: its `AttemptObservation` seam and its two imports
join the sweep's rewrite untouched, resolved as the union. P1 and P2 findings are fixed and P3 and lower
are recorded.

The file is test support, so §7's panic policy permits `expect` and `panic!` here and the review
that matters is §5, §12 and the correctness of what the fixture builds. It is also the fixture
every `workspace_manager` test rests on, and the five files under `src/engine/topology/` that
reach into it — `scaffold.rs` and the `attempt`, `dispatch`, `create` and `recover` suites — so a
weakness here weakens all of them at once.

**The finding that decided the shape of the change is `PR135-FIXTURE-AMBIENT-GIT-ENVIRONMENT-\
REDIRECTS-EVERY-COMMAND`.** Measured on git 2.43.0: with `GIT_DIR` set in the environment,
`git -C <fresh> init -q -b main --object-format=sha1` creates no repository at `<fresh>` at all.
It re-initialises the repository `GIT_DIR` names, warns `re-init: ignored --initial-branch=main`,
and exits 0 — and `git()` asserts only on the exit status. Every command after it then reads and
writes that repository: `git config` edits its config, `git add -A` stages from `GIT_WORK_TREE`,
`git commit` commits into it, and `rev-parse HEAD` answers with its head. A suite started from a
Git hook, from `git rebase --exec` or from `git bisect run` has those variables set, so this is
the ordinary way a developer runs a test suite from inside Git. `effects/allowlist.toml`'s
containment clause for this file — "Every write and every directory creation is under a root from
`scratch`" — is false while they are.

So the change gives the module the shape the funnel it supports already has: **one place where
every Git command is built**, mirroring `WorkspaceManager::command`, where the configuration and
the environment are decided once. Git's repository-locating environment is removed there, and the
settings an ambient config could otherwise use to change what a test observes are pinned there:
the hooks path, the fsmonitor, the attributes and excludes files, signing, and the six identity
variables. The module doc states each pin with its mechanism, and states the three inputs that
stay inherited on purpose — the `git` binary on `PATH`, the system config file, and object
replacement — with the reason for each.

Four more rows are the same class one level down. An ambient `core.hooksPath`, or a hook a
template directory copied into `.git/hooks` at `git init`, ran during the fixture's own `checkout`
and `commit` and wrote into the tree every later assertion reads (measured: a `post-checkout` hook
fires during `git worktree add` and `git checkout` and creates a file in the new tree). The
`user.name` and `user.email` the fixture wrote into the repository were not what decided the
committer, because environment identity overrides repository config — which the parent's
`git_with_identity` already says about its own commands. An attributes file's `text` or `eol`
attribute overrides `core.autocrlf=false`, so the line-ending pin the file already had was
incomplete. And `scratch` discarded its pre-clean's error while creating with `create_dir_all`,
which answers `Ok` for a directory that is already there, so a tree a previous run left at the
same predictable name — and a kill child always leaves one, because it dies by
`std::process::abort()` and its `Drop` never runs — was adopted as a fresh fixture in silence.

The better-implementation limb is the same commit: `git_command`, `scratch_at` separated from the
naming so the pre-clean is testable, `with_execution_root` for the one step two constructors
differ by nothing in, an `ObjectFormat` enum in place of the string `git init --object-format`
takes, and `SCRATCH` made private. The tests limb is fourteen tests and three `#[ignore]`d
children where the file had none — a Linux count, two of the fourteen being `#[cfg(unix)]` — each
witnessed under a stated mutation run on this box; the claims no test on this box can witness are
named in the pull request body rather than left to read as covered.

Two rows are deferred to the sweep of `src/workspace_manager.rs`, queue row 11: the parent's
`read_only_git` and `WorkspaceManager::command` inherit the same repository-locating environment
this file now strips, so `derive` under an ambient `GIT_DIR` computes its `repo_key` from another
repository's common git dir; and this module's `write_file` and `create_dir` take any path a
caller offers, so the allowlist's containment clause rests on caller discipline rather than on a
token of the kind `rundir`'s scratch tree carries. A third is deferred to the same row: the
fixture's three commits are `String` where the tree now has a validated `ObjectId`.

The first frontier pass, on `af53b8f` (posted 2026-09-04 as a pull request comment), returned
`CHANGES_REQUIRED` with six unlabelled findings. The coordinator classed 1, 2, 3 and 6 P2, and 4
and 5 §8 MUST deviations in code this pull request rewrote, which are fixed whatever their label.

**Three of the six are one finding, and the repair closes the class rather than the instances.**
The sweep had pinned ambient Git settings one at a time, and the pass simply named settings the
list did not: a template `config` carrying `core.worktree`, `i18n.commitEncoding`,
`protocol.file.allow`, an attributes file. That supply is unbounded. So `git_command` now clears
the environment and rebuilds it from an allowlist, and neutralises the *sources* — no config file,
no template, no system attributes, no home to find one under — which makes the claim a
construction rather than a list somebody has to keep complete. The second half of finding 3 is a
real boundary and is withdrawn rather than repaired: the door covers this module's commands and
not `WorkspaceManager::command`'s, and the row below carries its own P2 rather than a softened
one.

The repair introduced a defect of its own and it is a row here too: putting `scratch` on §8's
token left an adopted fixture holding none, so a kill child's tree outlived the parent that
adopted it. The coordinator refused it as a deferred row — correctly, on this box, whose inodes a
leaked temporary directory per fixture has already exhausted once — and it is fixed by having the
parent mint the tree before the child exists and keep the guard.

One row is routed rather than found here. `sweep-tests-25`, sweeping
`src/workspace_manager/tests.rs`, found that `died_by_abort`'s Windows arm accepts
`std::process::exit(1)`, and measured the part that matters first: no Unix leg can witness it,
because the Unix arm names `SIGABRT`. Its design is the repair — compare against a real abort
measured at run time rather than against a constant nobody here has observed — and it declined to
supply a Windows code it had not measured, which is the same discipline this pull request has been
applying to Git's behaviour all round.

| ID | Severity | Reviewed SHA / location | Failure sequence | Provenance | Category | First bad / prior ID | Regression or documented guard | Disposition |
|---|---|---|---|---|---|---|---|---|
| PR135-FIXTURE-AMBIENT-GIT-ENVIRONMENT-REDIRECTS-EVERY-COMMAND | P1 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:44 | `git_out` built `git -C <dir> <args>` with this process's whole environment, and `git` asserted only on the exit status -> with `GIT_DIR` set, `git -C <base> init -q -b main --object-format=sha1` creates no repository at `<base>`: it re-initialises the repository `GIT_DIR` names, warns `re-init: ignored --initial-branch=main` and exits 0 (measured, git 2.43.0), after which `git config` edits that repository's config, `git add -A` stages from `GIT_WORK_TREE`, `git commit` commits into it and `rev-parse HEAD` answers with its head -> a suite started from a Git hook, `git rebase --exec` or `git bisect run` writes the fixture's three commits into the developer's own checkout and every assertion after that is about that repository, while `effects/allowlist.toml`'s containment clause for this file is false | pre_existing | correctness | 61529ab | fixed: `git_command` is the one builder and removes every name in `LOCAL_ENV_VARS`, Git's own `rev-parse --local-env-vars` list. `the_fixture_is_immune_to_the_ambient_git_environment` runs `ambient_environment_child` with `GIT_DIR`, `GIT_WORK_TREE` and `GIT_INDEX_FILE` pointed at a second fixture and requires the child's three commit ids to equal this process's and the victim's `main` to be where it was; `the_variables_git_calls_local_are_the_ones_the_fixture_strips` asks the installed Git for its own list so the constant cannot age. Witnessed failing with the `env_remove` loop deleted and, separately, with `GIT_DIR` deleted from the constant | fixed |
| PR135-FIXTURE-AMBIENT-HOOKS-RUN-DURING-ITS-OWN-CHECKOUTS | P2 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:126 | the fixture set no `core.hooksPath`, so `git commit` and `git checkout -b side` ran whatever the ambient `core.hooksPath` named, and whatever `init.defaultBranch`'s neighbour `init.templateDir` or `GIT_TEMPLATE_DIR` had copied into `.git/hooks` at `git init` -> measured on git 2.43.0, a `post-checkout` hook reached that way runs during `git checkout` and `git worktree add` and writes a file into the tree -> the repository every `workspace_manager` suite measures gains an untracked file the next `git add -A` commits, and a hook is an effect no site accounts for inside a funnel whose whole claim is that every effect has one; the manager's own commands were already immune because `WorkspaceManager::command` pins the hooks path, so only the fixture's were exposed | pre_existing | correctness | 61529ab | fixed: `git_command` pins `core.hooksPath` at a name nothing creates. `a_hook_in_the_repositorys_own_hooks_directory_never_runs` plants an executable `post-checkout` in `.git/hooks`, runs two checkouts through the fixture and requires the marker to be absent, and `ambient_config_child` covers the global-config path with a hooks directory of its own; witnessed failing with `core.hooksPath` deleted from the builder | fixed |
| PR135-FIXTURE-IDENTITY-PINNED-WHERE-THE-ENVIRONMENT-OVERRIDES-IT | P2 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:102 | the committer was pinned as `user.email` and `user.name` in the repository's config -> environment identity overrides repository and global config, which `WorkspaceManager::git_with_identity` states for the manager's own commands, so an ambient `GIT_COMMITTER_NAME`, `GIT_COMMITTER_EMAIL` or `GIT_COMMITTER_DATE` decided instead -> the fixture's `seed`, `head` and `side` were a function of the machine rather than of their content, a malformed ambient date failed the whole suite, and nothing pinned the dates at all so no two runs produced the same object ids | pre_existing | correctness | 61529ab | fixed: `git_command` sets all six of `GIT_AUTHOR_NAME`, `GIT_AUTHOR_EMAIL`, `GIT_AUTHOR_DATE`, `GIT_COMMITTER_NAME`, `GIT_COMMITTER_EMAIL` and `GIT_COMMITTER_DATE`, so the three commits are the same ids on every machine, and the repository config is kept for a command run by hand. `the_fixture_is_immune_to_the_ambient_git_environment` hands the child a hostile identity and date and requires its ids to equal this process's; witnessed failing with the six settings deleted | fixed |
| PR135-FIXTURE-ATTRIBUTES-FILE-DEFEATS-THE-LINE-ENDING-PIN | P2 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:111 | `core.autocrlf=false` and `core.eol=lf` were pinned in the repository's config against exactly the Windows-guest case the comment beside them describes -> an attributes file's `text` and `eol` attributes override both, and `core.attributesFile` and the system `gitattributes` are ambient -> a machine with `* text eol=crlf` in its user attributes file checks the seed files out as CRLF while the blob is the one the fixture wrote, which is the same silent content change the pin exists to refuse, on the platform it was written for | pre_existing | portability | 61529ab | fixed: `git_command` pins `core.attributesFile` at a name nothing creates and sets `GIT_ATTR_NOSYSTEM`. `ambient_config_child` is given a global and a system config file naming an attributes file of `* text eol=crlf` and requires `b.txt`, the file the fixture's last checkout creates and therefore the one that goes through the attribute filter, to read back as it was written; witnessed failing with `core.attributesFile` deleted from the builder, after a first form of the witness that asserted on `a.txt` survived it because no checkout rewrites that file. The `GIT_ATTR_NOSYSTEM` half is not witnessed, because a test cannot install a system gitattributes file, and the pull request body says so | fixed |
| PR135-FIXTURE-SCRATCH-ADOPTS-A-TREE-IT-COULD-NOT-REMOVE | P2 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:39 | `scratch` opened with `let _ = fs::remove_dir_all(&dir)` and then `fs::create_dir_all`, which answers `Ok` for a directory that is already there -> a removal that failed for a permission error, a held file or a link loop was indistinguishable from one that succeeded, and the name is predictable: tag, process id and ordinal, and a process id is reusable -> a tree a previous run left at that name, which a kill child always leaves because it dies by `std::process::abort()` and its `Drop` never runs, became this run's fixture in silence, with that run's worktrees, intents and residue in it; `effects/allowlist.toml` records the deletion half of this hazard and not the adoption half | pre_existing | correctness | 61529ab | fixed: `scratch_at` inspects the removal's error, making only `NotFound` absence, and creates with the exclusive `fs::create_dir`. `a_stale_tree_that_cannot_be_removed_is_reported_and_not_adopted` makes the holding directory unwritable and requires the panic to name the previous run's tree, refusing to pass if it is running as a user for whom mode bits do not bind, and `scratch_empties_a_directory_a_previous_run_left_at_the_same_name` covers the ordinary path; witnessed failing with the two calls restored and, separately, with the pre-clean aimed elsewhere | fixed |
| PR135-FIXTURE-KILLABLE-CHILD-LEAKS-A-RUNNING-GIT-ON-PANIC | P2 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:285 | `KillableGitChild` owned a `std::process::Child` and had no `Drop` -> `Child` neither kills nor reaps on its own drop, and the sampler that owns these values reports every one of its assertions by panicking between `spawn` and `wait` -> a real `git add` or `write-tree` child kept running in a worktree the harness then removed, holding its index lock and its open handles, which is a removal failure on Windows for a reason that is not the one under test and an unreaped process on Unix; §6 asks RAII to clean up on panic unwinding as well as on the ordinary path | pre_existing | correctness | 61529ab | fixed: `impl Drop for KillableGitChild` kills and reaps, and both results are dropped with the reason on the line. `a_killable_git_child_is_killed_and_reaped_when_it_is_dropped` spawns a `git` alias that outlives the test, takes the child's process id through the new `id` accessor and requires the operating system not to know it after the drop; Unix only, because `kill` with signal 0 is the question and the pull request body says the Windows leg is unmeasured | fixed |
| PR135-FIXTURE-GIT-ANSWER-IS-A-LOSSY-STRING-USED-AS-AN-IDENTITY | P2 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:61 | `git` returned `String::from_utf8_lossy(&output.stdout).trim().to_owned()` -> a byte Git meant came back as `U+FFFD`, and this value is what the fixture and the five files under `src/engine/topology/` that use it read as a commit, a ref name and a path -> two different answers compare equal and a path that is not UTF-8 is silently a different path, which §8 refuses: a lossy display string is for diagnostics only and never identity; the same fold the parent removed from `decode_git_path` in the `parsers.rs` sweep, still here in the fixture that feeds it | pre_existing | correctness | 61529ab | fixed: `git` decodes with `String::from_utf8` and panics naming the command and the byte index where the answer stops being UTF-8, and the failure message now quotes stdout as well as stderr because Git reports on both. `git_refuses_an_answer_that_is_not_utf8` commits a two-byte file that is not UTF-8 and reads it back through `cat-file`; witnessed failing with the lossy decode restored | fixed |
| PR135-FIXTURE-ADOPT-READS-MAIN-TILDE-ONE-FOR-THE-FIRST-COMMIT | P2 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:163 | `adopt` derived `seed` as `rev-parse main~1` for a field documented as the first commit -> the two agree only while `main` has exactly two commits, and a kill child that committed on `main` before it died leaves three -> the parent that adopts the child's tree speaks about a different commit than the one the child built its worktrees at, and every cherry-pick and snapshot assertion downstream of `seed` is about that other commit, with nothing saying so | pre_existing | correctness | 61529ab | fixed: `adopt` reads the root commit through `--max-parents=0` and refuses an answer with more than one root. `adopt_re_derives_the_first_commit_after_main_has_moved_on` commits a third commit on `main` and requires the adopted `seed` to equal the original's while the adopted head does not; witnessed failing with the parent-of-main read restored | fixed |
| PR135-FIXTURE-CHILD-THAT-RAN-NO-TEST-READS-AS-A-COMPLETED-INJECTION | P2 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:264 | `run_kill_child` re-executed this binary with `--exact <test> --ignored` and returned only the exit status, and its streams were `Stdio::null()` while it passed `--nocapture` -> a filter that matches nothing runs nothing and exits 0 (measured), which is the same status a completed child gives -> `kill_child_and_adopt`'s assertion then reports a renamed or un-`#[ignore]`d child as `the child must have died by std::process::abort()` and blames the injection, which is a diagnosis of a regression that is not there; the same shape for a child that panicked in its setup, whose diagnostic was thrown away with its streams | pre_existing | correctness | 61529ab | fixed: `run_child_test` captures both streams, requires the harness prologue `running 1 test` and refuses the panic exit code, quoting what the child said in both messages. `a_child_whose_harness_selected_no_test_is_not_a_child_that_succeeded` and `a_child_that_panicked_says_so_and_quotes_what_it_said` are the two witnesses, each run against a child of its own; witnessed failing with each assertion deleted in turn | fixed |
| PR135-FIXTURE-EXITED-FOLDS-A-FAILED-POLL-INTO-STILL-RUNNING | P3 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:337 | `exited` matched `Ok(Some(_))` and folded every other answer, `Err` included, into `None` -> a `try_wait` that failed reported the child as still running -> the sampler polls to its deadline and then recalibrates from the runs that finished, so a failed poll removes a real measurement from the recalibration and reads as an over-long schedule; §7 does not let a failure become an absence | pre_existing | correctness | 61529ab | fixed: `exited` matches `Ok(None)` and panics on `Err` naming the step. No test witnesses it: a failing `try_wait` needs an operating-system level `waitpid` failure this fixture cannot induce portably, and the pull request body says so rather than leaving the row to read as covered | fixed |
| PR135-FIXTURE-TASK-BUILDS-A-SLOT-THE-MANAGER-REFUSES | P3 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:198 | `task` built `Slot::Task` from the variant's public fields, going around `Slot::validate` -> a key that is not a `safe_component` produced a slot the manager refuses at every primitive -> the test that asked for it measures a refusal it did not mean to ask for, and the failure names the primitive rather than the fixture step that built the bad value | pre_existing | correctness | 61529ab | fixed: `task` calls `validate` and panics naming the fixture step and the manager's own objection. `a_task_key_the_manager_would_refuse_fails_at_the_fixture_step` asks for a key with a separator; witnessed failing with the check deleted | fixed |
| PR135-FIXTURE-SCRATCH-TAG-IS-NOT-ONE-PATH-COMPONENT | P3 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:33 | `scratch` interpolated its `tag` into a directory name with no validation, and created it with `create_dir_all` -> a tag carrying a separator makes a nested path, which `create_dir_all` happily creates -> `Fixture::drop` removes the leaf it was given and leaves the directory the tag's first component made, so the temporary directory accumulates trees nothing owns, and two tags sharing a first component share a parent | pre_existing | correctness | 61529ab | fixed: `scratch` validates the tag through the manager's own `safe_component`, the same predicate that makes a `Slot` path containment-by-construction. `a_scratch_tag_that_is_not_one_path_component_is_refused` asks for a tag with a separator; witnessed failing with the check deleted | fixed |
| PR135-FIXTURE-AMBIENT-SIGNING-AND-IGNORE-FILE-FAIL-THE-SUITE | P3 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:118 | the fixture pinned neither `commit.gpgsign` nor `core.excludesFile` -> a machine with signing on fails every fixture commit, and one whose global ignore file excludes the seed files leaves `git add -A` with nothing to stage so `git commit` exits non-zero -> the whole `workspace_manager` suite is red on that machine for a reason that is nothing to do with the code, which §12 asks a hermetic test not to depend on; loud rather than silent, which is why this is a P3 and not one of the rows above | pre_existing | correctness | 61529ab | fixed: `git_command` pins `commit.gpgsign` off and `core.excludesFile` at a name nothing creates. `ambient_config_child` is given a global and a system config file that turn signing on and exclude the seed files; witnessed failing with each pin deleted in turn | fixed |
| PR135-FIXTURE-OBJECT-FORMAT-IS-A-STRING-TAG | P3 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:93 | `with_object_format` took the format as `&str` and `created_sha256` repeated `created`'s body around it -> §5 asks for an enum rather than a string tag where the alternatives differ in meaning, and the difference here is what a test may assert about an object id -> a caller could ask for a format `git init` refuses, and the hash length each format implies was spelt at every site that needed it instead of once | pre_existing | docs-contract | 61529ab | fixed: `ObjectFormat` with `Sha1`, `Sha256` and `hex_len`, and `with_execution_root` for the one step the two `created` constructors differ by nothing in. `the_object_format_is_the_one_the_fixture_asked_for` builds a repository of each and requires the head's length to be that format's; witnessed failing with the argument ignored | fixed |
| PR135-FIXTURE-PANICS-DO-NOT-NAME-THE-STEP | P3 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:50 | the `expect` messages were `run git`, `create the scratch directory`, `seed file`, `write the intent` and the like -> none named the value it was working on, and several named neither the step nor the file -> a fixture that fails anonymously turns a defect in the code under test into a confusing failure in the setup, which §7 asks a test's own `expect` not to do | pre_existing | docs-contract | 61529ab | fixed as text: every `expect` and `panic!` in the file names the fixture step and its value, and the module doc states the contract once so a new one has something to match. `a_task_key_the_manager_would_refuse_fails_at_the_fixture_step` and `a_scratch_tag_that_is_not_one_path_component_is_refused` assert on the text of two of them | fixed |
| PR135-FIXTURE-HAS-NO-MODULE-DOC-AND-NO-TESTS | P3 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:1 | the file opened with the allowlist prologue and no `//!` doc, and had no test of its own -> nothing said what the fixture guarantees, and nothing executed it: the object format was pinned with a comment and no assertion, and `SCRATCH` was `pub(crate)` for no caller -> every claim the fixture makes rested on the suites that use it noticing, and those suites are exactly the ones a broken fixture makes pass | pre_existing | docs-contract | 61529ab | fixed: a module doc listing every pin with its mechanism and the three inputs inherited on purpose, `SCRATCH` made private, and fourteen tests and three `#[ignore]`d children in the file, a Linux count with two of the fourteen `#[cfg(unix)]`. `the_object_format_is_the_one_the_fixture_asked_for` is the pin that had a comment and no assertion | fixed |
| PR135-PARENT-READ-ONLY-GIT-INHERITS-THE-REPOSITORY-LOCATING-ENVIRONMENT | P3 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager.rs:3108 | `read_only_git` and `WorkspaceManager::command` build their commands with this process's whole environment -> `common_git_dir` runs `rev-parse --path-format=absolute --git-common-dir` through `read_only_git`, so under an ambient `GIT_DIR` it answers with the repository that variable names -> `WorkspaceManager::derive` computes `repo_key` and therefore the execution root from another repository's common git dir while managing this one, and every funnel command afterwards runs against a repository the caller did not name; the fixture's own commands are immune from this pull request, the parent's are not | pre_existing | correctness | 61529ab | deferred to the sweep of `src/workspace_manager.rs`, queue row 11, which owns both helpers. The reproduction is the fixture's `the_fixture_is_immune_to_the_ambient_git_environment` with the strip moved to the parent, and its child already runs `derive` under a hostile `GIT_DIR` today | deferred |
| PR135-FIXTURE-WRITE-HELPERS-TAKE-ANY-PATH | P3 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:235 | `write_file` and `create_dir` write wherever their caller points them -> `effects/allowlist.toml` claims for this file that every write and every directory creation is under a root from `scratch`, and nothing enforces it -> the claim is caller discipline read as a property, and the six suites that call these helpers are where a mistake would land; no caller does so today | pre_existing | correctness | 61529ab | deferred to the sweep of `src/workspace_manager.rs`, queue row 11, with a proposal: a scratch-root token of the kind `rundir`'s scratch tree already carries, taken by these helpers and issued only by `scratch`. The allowlist row is corrected in this pull request to say what is enforced and what is discipline | deferred |
| PR135-FIXTURE-COMMITS-ARE-STRINGS-WHERE-THE-TREE-HAS-OBJECTID | P3 | 95c5bd336986a620f1b36c2e17496717c0edae6c / src/workspace_manager/fixture.rs:75 | `seed`, `head` and `side` are `String` and the struct's fields are `pub(crate)` -> the tree now has a validated `ObjectId`, which `snapshot_ref.rs`'s sweep introduced for exactly this mix-up, and a ref name or a short id is the same type as a full object id here -> a suite can hand the manager a value the production types would refuse and measure a refusal rather than the behaviour it meant to, which is the trap this sweep's brief names; every value these fields hold today comes from `rev-parse` and is a full id | pre_existing | docs-contract | 61529ab | deferred to the sweep of `src/workspace_manager.rs`, queue row 11, where `snapshot_ref.rs`'s own deferred row already proposes consolidating `ObjectId` beside its predicates and giving the ref primitives and the engine's `CommitSha` the same type; the fixture's three fields belong in that change and not in a separate one | deferred |
| PR135-REVIEW1-AMBIENT-CONFIG-AND-TEMPLATE-ESCAPE-THE-PINS | P2 | af53b8f0a457522b165c5851c7026099a34ef591 / src/workspace_manager/fixture.rs:294 | the sweep pinned ambient settings one at a time -> pass 1 walked in through each one the list did not name: a `GIT_TEMPLATE_DIR` template whose `config` sets `core.worktree`, which made the new repository resolve its worktree to another and stage that one's file (measured at the exact head, and `the_object_format_is_the_one_the_fixture_asked_for` failed at the second commit with "nothing to commit"); `i18n.commitEncoding=ISO-8859-1`, which puts an `encoding` header on every commit and changes every object id; and `protocol.file.allow=never`, which the file's own doc said no command needed while `src/workspace_manager/tests.rs` runs `git fetch <local-path>` through this module -> the containment claim and the body's "only three ambient inputs remain inherited" were both false, and the supply is unbounded, so a fourth pass would have found a fifth setting | pre_existing | correctness | 61529ab | fixed: `git_command` clears the environment and rebuilds it from `INHERITED`, and neutralises the sources rather than the settings -- `GIT_CONFIG_GLOBAL`, `GIT_CONFIG_SYSTEM`, `HOME` and `XDG_CONFIG_HOME` at an absent path, `GIT_CONFIG_NOSYSTEM`, `GIT_ATTR_NOSYSTEM`, and an empty `GIT_TEMPLATE_DIR`. `the_fixture_is_immune_to_an_ambient_config_and_template` attacks with all three escapes at once plus a template `post-checkout` and `info/attributes`, and `the_only_config_a_fixture_command_reads_is_the_repositorys_own` asserts through `--show-origin` that no source outside the repository is read at all | fixed |
| PR135-REVIEW1-MANAGER-CHECKOUT-IS-NOT-COVERED-BY-THE-ATTRIBUTES-PIN | P2 | af53b8f0a457522b165c5851c7026099a34ef591 / src/workspace_manager.rs:2898 | the body claimed the attributes pin protected the manager's checkouts -> `WorkspaceManager::command` sets neither `core.attributesFile` nor `GIT_ATTR_NOSYSTEM`, and this module's door covers only the commands it builds -> an ambient `* text eol=crlf` still rewrites what `git worktree add` writes under `Fixture::add_task`, and the fixture's own witness checks only the fixture's own checkout, so the claim was unwitnessed and unimplemented | pre_existing | portability | 61529ab | the claim is **withdrawn** rather than repaired: the parent's command builder is queue row 11 and unswept, so under `MAINTAINING.md` this is logged. The severity here is the finding's own and is not softened for being deferred. The module doc, `Fixture::adopt`'s neighbour comment in `ambient_config_child` and this row all name `WorkspaceManager::command` and both gaps, so fixture hardening cannot be read as manager hardening | deferred |
| PR135-REVIEW1-SCRATCH-DELETES-A-TREE-IT-CANNOT-PROVE-IT-OWNS | P2 | af53b8f0a457522b165c5851c7026099a34ef591 / src/workspace_manager/fixture.rs:231 | `scratch_at` derived a name from the tag, the process id and an ordinal and called `remove_dir_all` on it before acquiring anything -> after process-id reuse, or with two process-id namespaces sharing one temporary directory, that destroys an earlier occupant's tree, and creating exclusively afterwards does not authorise the deletion that preceded it -> §8 requires recursive deletion of a run-scoped tree to be token-carried and names the `cfg(test)` scratch-tree token for exactly this; the previous round disclosed the deletion instead, and disclosure does not make a rewritten function compliant | pre_existing | correctness | 61529ab | fixed: `scratch_tree` acquires through `rundir::scratch_tree`, whose name carries a fresh ULID, whose create is exclusive, and which pre-cleans nothing; the removal is spent against a token bound to that exact root. `a_second_scratch_tree_is_a_second_root_and_pre_cleans_nothing` and `a_dropped_scratch_guard_reclaims_its_own_tree_and_nothing_else` pin both halves, and the type system pins the rest: a `ScratchTree` cannot be constructed outside that module, so this file cannot pre-clean and still return a guard | fixed |
| PR135-REVIEW1-GIT-TRIM-EATS-A-TRAILING-SPACE | P2 | af53b8f0a457522b165c5851c7026099a34ef591 / src/workspace_manager/fixture.rs:355 | `git` ended with `trim()` -> `rev-parse --show-toplevel` in a repository whose path ends in a space answers with that space and then the line terminator, and trimming whitespace removes both -> the caller gets a different path, which is the identity rule §8 states and the previous round's own fix claimed to have satisfied | pre_existing | correctness | 61529ab | fixed: one line terminator is stripped, `\r\n` included, and nothing else; the decode stays strict so the value is validated rather than assumed. `git_strips_the_line_terminator_and_not_a_trailing_space` builds a repository whose path ends in a space and round-trips it, Unix only because Windows strips a trailing space from a directory name itself | fixed |
| PR135-REVIEW1-CONFIG-VALUE-IS-A-DISPLAY-STRING | P2 | af53b8f0a457522b165c5851c7026099a34ef591 / src/workspace_manager/fixture.rs:1315 | the hostile-config witness built its file with a `Path` to display-`String` conversion -> a non-Unicode Unix path or an ill-formed Windows `OsString` is replaced before the value is written into Git config, and Git then reads a path that is not the one meant -> §8 allows a display conversion for diagnostics only, and this value is read back as a path | pre_existing | correctness | 61529ab | fixed: `config_path_bytes` writes the path's own bytes on Unix and refuses a non-Unicode path elsewhere rather than replacing it, escaping Git's `\` on the bytes; the config file is assembled as `Vec<u8>` | fixed |
| PR135-REVIEW1-CHILD-CAPTURE-IS-UNBOUNDED-AND-UNTIMED | P2 | af53b8f0a457522b165c5851c7026099a34ef591 / src/workspace_manager/fixture.rs:676 | `run_child_test` piped both streams into `Command::output` -> the buffering is unbounded and there is no timeout, so a child that loops while logging exhausts this process before its status can be reported and a wedged child never returns -> newly worse than the `Stdio::null()` it replaced, against §9's size and timeout requirements | introduced_by_feature | liveness | 61529ab | fixed: `read_capped` keeps 64 KiB of each stream, reads past the bound so the child is never blocked on a full pipe, and tells the caller what it dropped; the wait and both joins carry `CHILD_DEADLINE`, and a child that overruns it is killed, reaped and reported rather than returned as an ordinary status. `a_childs_stream_is_captured_up_to_the_bound_and_read_past_it` and `a_reader_that_does_not_finish_inside_the_bound_is_reported` are the witnesses; the deadline's own expiry is not exercised by a test and the body says so | fixed |
| PR135-ROUTED-DIED-BY-ABORT-IS-A-NEGATION-ON-WINDOWS | P2 | af53b8f0a457522b165c5851c7026099a34ef591 / src/workspace_manager/fixture.rs:848 | `died_by_abort`'s Windows arm was `!status.success() && status.code() != Some(PANIC_EXIT)` -> every unsuccessful exit that is not a panic reads as an abort, `std::process::exit(1)` included -> `kill_child_and_adopt` accepts a child that failed for any reason as a child the injection killed, so the oracle has stopped discriminating and the suite has stopped detecting; no Unix leg can see it, because the Unix arm names `SIGABRT`, which is why this file's own catalogue never witnessed it. Found by `sweep-tests-25` while sweeping `src/workspace_manager/tests.rs` and routed here because this file is this pull request's subject; its own row is `PR136-PASS2-P3-DIED-BY-ABORT-IS-A-NEGATION-FOR-ITS-OTHER-CALLER` | pre_existing | correctness | 61529ab | fixed: the Windows arm compares against a **measured** abort rather than an encoded code -- `measured_abort_end` runs one real `std::process::abort()` and remembers how it ended -- so a runtime that changes the code cannot make the oracle stale. The design is `sweep-tests-25`'s, which declined to give a number it had not observed. `the_abort_oracle_separates_an_abort_from_an_ordinary_failure` runs both children on every platform, asserts its own premises, writes the old negation out inline and asserts it accepts both, and pins that a panic is still refused | fixed |
| PR135-REPAIR-ADOPTED-TREE-OUTLIVED-ITS-ADOPTER | P2 | 4352156d939e72aa59acfe5f907f0969dbf31d6f / src/workspace_manager/fixture.rs:591 | putting `scratch` on the §8 token left `Fixture::adopt` holding no token, since one cannot be minted from a path -> an adopted fixture removed nothing, so the tree a kill child leaves behind outlived the parent that adopted it, where `Fixture::drop` had removed it before -> a temporary directory leaked per kill test on a box whose inodes exactly that has already exhausted, with `df` reporting free space while every write failed; a regression this pull request introduced, caught by the coordinator's read rather than by a gate | fix_regression | liveness | 4352156 | fixed: the parent mints the tree before the child exists and keeps the guard. `Fixture::under` builds in a subtree of a parent the caller names, `Fixture::adopt` takes that guard, and a child that exits normally still reclaims its own subtree. The parent directory is passed as an argument and never read from the environment here, because a variable read in this file would be an ambient input in the module whose claim is that it trusts none; `scaffold.rs` owns the kill protocol's variable and reads it | fixed |
