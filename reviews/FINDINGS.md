...
| PR119-MACOS-PROC-SUSPEND-CONTINUE-EXIT-BUDGET | One macOS (`test (macos-latest)`) run failed `agent::proc::tests::terminal_suspend_and_continue_cover_the_isolated_tree` at the **exit wait after `SIGCONT`**, not at the suspension-coverage assertion the test exists for: `wait_for_exit(&mut helper.child, Duration::from_secs(10))` returned `None` and the `expect("continued helper completes normally")` at `src/agent/proc.rs:8202` panicked; `test result: FAILED. 1806 passed; 1 failed; 34 ignored; finished in 242.38s`. The same commit passed the same leg on a rerun in place. No rate has been measured and the test name appears in no earlier record | project owner / the slice that next opens `src/agent/proc.rs`, once a controlled macOS environment can measure it | **Open as one unexplained observation on a platform-timing budget, not classified as a flake or regression.** **The failure.** Run `33798030529`, attempt 1, job `100790356980`, at `4094c57973ffd7f76a9868d634296be8b0f9a3f1` (PR #119). A normal assertion failure with a `failures:` section and a `test result:` line, so not the `PR43-MACOS-PROC-SIGNAL-FINGERPRINT` shape (`exit status: 143`, a different test) and not the C-004 SIGTERM shape. **The rerun.** `gh run rerun 33798030529 --failed` re-ran only the failed job at the byte-identical head: attempt 2, job `100810497995`, `test (macos-latest)` **success**, the test `ok`, `1807 passed; 0 failed; 34 ignored; finished in 288.63s`. One failed and one green attempt at one head is what makes this nondeterministic rather than a defect in the head, and that opportunistic pair is not promoted to a rate. **What the failed assertion depends on, read from the source and not measured.** The earlier assertions in the same test passed, so the helper was observed stopped and the isolated worker made no progress while suspended (`8182` to `8194` at this head). The assertion that failed is the 10 s bound at `8201` on the helper's *exit* after `SIGCONT`, which spans a chain of independent schedulings: the helper's `SIGCONT` handler sets a flag (`2212`), a monitor thread polling every 10 ms forwards `SIGCONT` to the isolated group (`2442` to `2450`), the worker resumes and notices the `finish` file on its next 50 ms poll (`7339` to `7355`), the helper's `run_with_timeout` reaps it and asserts exit 0 (`7569` to `7570`), and the helper, itself a `cargo test` harness running one ignored test, prints its result and exits. On a `macos-latest` runner whose suite took 242 s with every other `agent::proc` signal test spawning helpers alongside, a fixed 10 s budget over five process hops is the class §12's readiness rule names: a deadline short enough to expire on a loaded runner has become the failure it exists to prevent. That is a reading of the code, offered as reasoning that does NOT satisfy a floor; the cause is not guessed here. **What the diff has to do with it.** The failing test is not one this diff touches: nothing under `src/agent/` changes. The diff does add seven tests to the same test executable, and whether that altered the scheduling the helper's budget depends on is not argued either way; one pass and one failure at one SHA show nondeterminism, not the absence of a defect at that head. The runner-load cause, a fixed READY budget for the forked helpers, is being fixed in PR #125. **What would settle it.** A rate: this one test in a loop on a macOS host or a CI job that reports failures over runs, fingerprinted by the `8202` assertion. **The repair fork, stated so the next owner does not re-derive it**: either the exit wait bounds a wedged producer rather than timing a healthy one (a longer bound, or a wait that watches the worker's marker stop advancing before timing the helper's exit), or the test asserts helper exit only after observing the worker's own exit; a repair measured only in isolation has not been measured. **Provenance is inline rather than in a companion record**, as for `PR104-WINDOWS-SETTLE-PATH-HINT-FINGERPRINT`, because this append is confined to `reviews/FINDINGS.md` in its own commit on the pull request's branch. |
...
Append-only. The §6/§7 sweep of `src/workspace_manager/hooks.rs` (PR #119) took two frontier
passes: `a1b319c` (three findings) and, after the finding-2 repair at `6a13e1d`, that head (six
findings). Both verdicts were `CHANGES_REQUIRED`, none P1. Under the owner's direction that a
