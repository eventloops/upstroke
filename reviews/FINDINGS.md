...
**It is not a completed sweep and the file is not in the swept table.** Six inspections still fold
an I/O failure the filesystem declined to explain into the same `Husk` as an honest absence, and
...
shipped. All five are fixed, and three of them are fixed by **withdrawing a claim rather than
changing code** — the swept-table entry, the design authority, and the constraint the fifo deferral
rested on. That is the shape of this pass: one real defect in the machinery the first round added,
and four places where this pull request said more than it could support. The rows are appended
below with the pull request's number as their prefix.
...
each remaining file as it is resolved.
...
(`SWEEP-CLASSIFY-005`). The remaining **six** — the stat of the name, the `open`, the `fstat` for
the bound, the two reads and the seek — fold an I/O failure the filesystem declined to explain into
the same `None` as "there is nothing here". Pass 1 caught this paragraph saying five while listing
six, and the miscount was the reason the swept claim looked survivable: six is the number that
decides it.

The rule this module implements classifies as `Committed` or "anything else", so `Husk` is the
answer for all six and this pull request does not change it — it corrects the claim that the file
...
**The first repair of the first of those did not work, and that is the most useful thing in this
section.** It bounded `newline_offset_from`, the scan *between* the probe's two reads, while both
reads went through `Read::read_to_end` — which retries `Interrupted` inside `std::io` without limit
and spends none of a `Take`'s byte budget doing it. Measured at rustc 1.85: a reader answering
`Interrupted` unconditionally was still being called after five million reads with the `Take` limit
untouched. So the allowance was never reached and the probe still hung on its first read. The
repair's own test fixture *documented* the hole — it fired its interruptions past the window "so a
schedule firing from byte zero would be spent there" — which is how a suite can be green over a
defect its author has written down. The bound is at the read now, in `read_chunk`, and the two
tests that fall into the hole rather than stepping over it are
...
| ID | Severity | Reviewed SHA / location | Failure sequence | Provenance | Category | First bad / prior ID | Regression or documented guard | Disposition |
|---|---|---|---|---|---|---|---|---|
| SWEEP-CLASSIFY-001 | P2 | 5f661fa7f8d5c45471cc33746a70df1cd192c61e / src/rundir/classify.rs:236 | the scan retried an `Interrupted` read with a bare `continue`, which is the one branch of the loop that spends no budget -> a source that answers `Interrupted` for ever is scanned for ever, and the function's own termination proof asserted "every iteration either returns or spends at least one byte of budget" two clauses before naming the branch that made it false -> `startup_census` holds the physical worktree lock across `classify_run_dir`, so a scan that does not return is the lock held for ever and no later command in that worktree can run. No claim is made that a regular file never returns `Interrupted`: an earlier version of this row said so, `std::io`'s contract does not, and a termination argument resting on an assumption about the source is what the bound exists to retire | pre_existing | liveness | 7a83e69 | fixed at the read, in the second round. **The first round's repair did not fix this and the row said it did** -- it bounded `newline_offset_from`, the scan between the probe's two reads, while both reads went through `Read::read_to_end`, which retries `Interrupted` inside `std::io` without limit and spends none of a `Take`'s byte budget, so `INTERRUPTED_RETRIES` was never reached; that recurrence is `PR137-CLASSIFY-INTERRUPTED-RETRY-NEVER-REACHED`. `read_chunk` is now the one place a read happens and the one place the allowance is spent, `read_within` replaces both `read_to_end` calls, and `newline_offset_from` shares the primitive. Three tests: `the_scan_retries_an_interrupted_read_and_stops_retrying` for the scan, `an_unconditionally_interrupted_source_has_no_first_line` for the first read and `a_source_that_is_interrupted_for_ever_after_the_rewind_has_no_first_line` for the re-read | fixed |
