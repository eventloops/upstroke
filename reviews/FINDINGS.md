...
| PR7-FOLD-ACCESSORS-IN-PR3-LAYER | `src/topology/fold.rs` is **+628 / −0** in PR7. Disclosed here rather than left for a reviewer to find, because it is PR3's file and the slice is large enough that a footprint that size can stop being visible to the person making it | project owner — **the post-v0.2 pass over PR3's layer** | **Recorded, and argued rather than assumed.** Measured split: **474 lines are tests**; of the 154 production lines, **about 130 are doc comments**. The code is **nine `pub fn` accessors** — `ready`, `ready_retry`, `pipeline_held`, `pipeline_reservable`, `structurally_admissible`, `integration_admissible`, `run_is_ending`, `backoff_pending`, `questions_open` — each a one-line delegation to an *existing private* `RunState` predicate with a poison guard, plus **one line of changed behaviour**: `&& self.pipeline_reservable()` in `integration_admissible`, which is round 1's HIGH, where selection breached `max_parallel` by one at the only width production creates. **Why this is not the shape `ff0490a` forbade:** that ruling names a new *variant* in `src/topology/effects.rs` and a *widened* `CommandSpec.program`, and its principle is "a slice may not quietly **redesign** what it implements". No variant is added, no type widened, nothing deleted; `select.rs` cannot ask the fold what `eligibility_order` requires it to ask without a public reader, and the conjunct is one `permits.pipeline` demands. The owner should still rule, which is why the row exists |
