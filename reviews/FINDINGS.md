...
## 50. PR #128 residue.rs sweep (2026-09-04)
...
| PR128-RESIDUE-RECORD-FOR-FOLDS-A-FAILED-WORKTREE-LIST | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager.rs:3063 | `record_for` answers `None` when `git worktree list --porcelain -z` exits non-zero -> the classifier's three add sites read `None` as not registered, so the after phase is absent and `RegisteredUnpopulatedWorktree` is absent -> `classify_object_residue` answers `None` for a repository Git could not open, the class no action recovers; a spawn failure is propagated, a failure that speaks is not; no production caller reaches the add sites today | pre_existing | correctness | 7a83e69 | fixed at 335bb27637f080a2800d2acb2a61d020716eb2e4: `record_for` reads the listing through `read_only_git_ok`, so a listing that failed is the error naming the command and the repository; `a_directory_that_is_not_a_repository_is_an_error_at_every_classified_site` drives every classified site against an empty directory and was witnessed failing at the three adds with the fold restored, and `the_classifiers_inspections_report_a_git_failure_instead_of_answering` drives `record_for` directly | fixed |
| PR128-RESIDUE-OBJECT-EXISTS-FOLDS-A-FAILED-CAT-FILE | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager.rs:3003 | `object_exists` reads any non-zero `cat-file -e` as no such object, while `WorkspaceManager::object_exists` promises "a Git error other than no such object" -> the two commit-tree sites' after phase and `CandidateWriteTree`'s read `false` -> `verify_object` in `src/engine/topology/candidate.rs` refuses `ObjectMissing` for a repository Git could not open, naming a lost candidate where the failure was Git's; the one production path through the classifier today | pre_existing | correctness | 7a83e69 | fixed at 335bb27637f080a2800d2acb2a61d020716eb2e4: `object_exists` asks `rev-parse --verify --quiet <object>^{}` through `read_only_verify`, the free twin of `quiet_object_lookup`, since `cat-file -e <object>^{}` exits 128 for a missing object and an unopenable repository alike (measured on git 2.43); the `# Errors` of `WorkspaceManager::object_exists` is now true; `the_classifiers_inspections_report_a_git_failure_instead_of_answering` asserts the missing-object `false`, the present `true` and the error for a directory that is not a repository, witnessed failing with `cat-file -e` restored | fixed |
| PR128-RESIDUE-HEAD-COMMIT-FOLDS-A-FAILED-REV-PARSE | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager.rs:3008 | `head_commit` answers `None` for any non-zero `rev-parse --verify --quiet HEAD` -> `ProposalCherryPick`'s after phase reads a Git failure as no HEAD -> the site classifies `None` or `Internal` for a worktree whose state is unknown; no production caller reaches the site today | pre_existing | correctness | 7a83e69 | fixed at 335bb27637f080a2800d2acb2a61d020716eb2e4: `head_commit` is `read_only_verify` on `HEAD`, so an unborn branch (exit 1 in silence, measured) is `None` and a repository Git cannot open is the error; `the_classifiers_inspections_report_a_git_failure_instead_of_answering` was witnessed failing with the fold restored | fixed |
| PR128-RESIDUE-INDEX-DIFFERS-READS-A-FAILED-DIFF-AS-THE-AFTER-PHASE | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager.rs:3051 | `index_differs_from_head` reads any non-zero `diff --cached --quiet` as differs, where exit 1 is the answer and 128 is a failure -> at `RepairMaterialize` a Git failure is the after phase -> `classify_object_residue` answers `After` for a worktree it could not inspect, the fold in the other direction: a failure read as success; no production caller reaches the site today | pre_existing | correctness | 7a83e69 | fixed at 335bb27637f080a2800d2acb2a61d020716eb2e4: `index_differs_from_head` matches the exit status, 0 and 1 being the answers and anything else the error (an empty directory answers 129, measured); `a_directory_that_is_not_a_repository_is_an_error_at_every_classified_site` was witnessed failing at `RepairMaterialize` with the fold restored, answering `After`, and `the_classifiers_inspections_report_a_git_failure_instead_of_answering` asserts both answers and the error | fixed |
| PR128-RESIDUE-UNREACHABLE-OBJECTS-IGNORES-THE-EXIT-STATUS | P3 | 80843302a8367e607e54f181ef592c02ca5a089f / src/workspace_manager.rs:2924 | `unreachable_objects` parses stdout and never reads the exit status of `git fsck` -> a fsck that failed lists nothing -> `UnreferencedObject` is never observed at any site, and `CandidateWriteTree`'s after phase reads a present tree as reachable; a failure read as no residue at every Object site; no production caller reaches these arms today | pre_existing | correctness | 7a83e69 | fixed at 335bb27637f080a2800d2acb2a61d020716eb2e4: `unreachable_objects` reads through `read_only_git_ok`, so an empty listing is the answer only when fsck exited 0 (which it does with unreachable objects present, measured); `the_classifiers_inspections_report_a_git_failure_instead_of_answering` asserts an unreachable blob is listed and an empty directory is the error, witnessed failing with the exit status ignored again | fixed |
...
| PR128-REVIEW-UNREADABLE-PACK-READS-AS-ABSENT-AND-AS-DIFFERING | P2 | b5c12e042b01798d7f0dc8bec15543da9d807a33 / src/workspace_manager.rs:2935 | `read_only_verify` took exit 1 with empty stderr as absence and `index_differs_from_head` took exit 1 as a difference -> with a pack file at mode 000 and its `.idx` readable, git 2.43 answers `rev-parse --verify --quiet <id>^{}` with exit 1 in silence and `diff --cached --quiet` with exit 1, and `cat-file --batch-check` prints `missing` (measured) -> `WorkspaceManager::object_exists` answered `false` and `refuse_absent_source` refused a missing candidate for a store it could not read, and `RepairMaterialize` classified a clean but unreadable repository `After`; the pull request's claim that an unreadable store propagates an error held only for a store Git itself refuses to open | introduced_by_feature | correctness | 335bb27 | fixed at 87c29fc9497fbe3bb3193ea8484cbab449e53cd0: Git gives the same answer for an object it cannot read as for one it does not have, so the absent and differing answers are taken only after `refuse_unreadable_object_store` finds the objects directory, every fan-out directory and the pack directory listable and every `.pack` and `.idx` openable (an unreadable loose object file and an alternate store are stated as outside the check); `an_unreadable_pack_is_an_error_and_not_a_missing_object_or_a_difference` repacks a fixture, sets every pack to mode 000 as uid 1000 with the bite asserted, and asserts the `Io` error naming the pack from `object_exists`, the manager's `object_exists`, `index_differs_from_head` and the classifier at `RepairMaterialize` and `CandidateCommitTree`; witnessed failing with the check removed from `read_only_verify` and, separately, from `index_differs_from_head` | fixed |
| PR128-REVIEW-ADD-STATE-DOES-NOT-ESTABLISH-A-GIT-DIRECTORY | P2 | b5c12e042b01798d7f0dc8bec15543da9d807a33 / src/workspace_manager/residue.rs:451 | `git_dir_of` returned `Some(PathBuf::new())` for a pointer reading `gitdir:` with an empty path and `Some` for any target without reading it -> `add_state` answered `Populated` for a registered worktree with no git directory behind its pointer, the state the pull request claimed to fix, and every later name was joined onto `""` -> the add sites classified `After`; git 2.43 refuses the empty pointer as "invalid gitfile format" | introduced_by_feature | correctness | a17b8c5 | fixed at 87c29fc9497fbe3bb3193ea8484cbab449e53cd0: `git_dir_of` takes the empty path as no directory by name, resolves a relative target against the worktree as Git does, and answers `Some` only for a target whose `symlink_metadata` is a directory (`None` for absent or a file, `Io` otherwise); `a_gitfile_that_names_no_directory_is_not_a_populated_worktree` executes Git's refusal and drives the empty path, a missing target and a file target through `git_dir_of` and the classifier, asserting `Internal` with the element observed; witnessed failing with the pointer's text trusted again | fixed |
| PR128-REVIEW-RECORD-FOR-READS-AN-UNLISTABLE-REGISTRATION-AS-ABSENT | P2 | b5c12e042b01798d7f0dc8bec15543da9d807a33 / src/workspace_manager.rs:3108 | `record_for` trusted a listing that exited 0 -> with a registration's administrative directory at mode 000, `git worktree list --porcelain -z` exits 0 with nothing on stderr and omits it (measured on git 2.43) -> `add_state` answered `Unregistered` and the add sites classified `None` for a registration the process could not read; exit status alone did not make the listing complete | introduced_by_feature | correctness | 335bb27 | fixed at 87c29fc9497fbe3bb3193ea8484cbab449e53cd0: before an omission is `None`, `refuse_unlistable_registrations` reads every registration under the common git dir's `worktrees/`, and one whose `gitdir` cannot be read is the `Io` error naming it; `an_unlistable_registration_is_an_error_and_not_an_unregistered_worktree` sets the administrative directory to mode 000 as uid 1000, executes the measurement (Git's listing omits the worktree, exit 0) and asserts the error from `record_for` and the classifier; witnessed failing with the scan removed | fixed |
| PR128-REVIEW-GIT-FAILURES-CARRY-NO-COMMAND-OR-STDOUT | P2 | b5c12e042b01798d7f0dc8bec15543da9d807a33 / src/workspace_manager.rs:2891 | `read_only_git`'s spawn failure said only "failed to run git", and `read_only_git_failure` carried stderr alone -> a corrupt repository makes `fsck` exit 2 with `missing blob <id>` on stdout and nothing on stderr (measured on git 2.43), so `unreachable_objects` returned an error with a blank diagnosis, and a spawn failure named neither command nor directory -> the §7 operation-context claim the body made for every propagated `?` was false at the source | introduced_by_feature | correctness | 335bb27 | fixed at 87c29fc9497fbe3bb3193ea8484cbab449e53cd0: the spawn failure names the command and the directory; `read_only_git_failure` names the command, the directory, the exit status (or the signal) and both streams, trimmed and bounded to `GIT_OUTPUT_BOUND` characters; `a_failed_git_inspection_names_the_command_the_directory_the_status_and_what_git_said` deletes a reachable loose blob and asserts the `fsck` error carries the command, the repository, `exit status 2` and `missing blob <id>`; witnessed failing with stdout dropped; the spawn failure is covered by construction, since making `git` unfindable means editing the process environment under parallel tests | fixed |
| PR128-REVIEW-INSPECTION-BEHAVIOUR-HAS-NO-DESIGN-SENTENCE | P2 | b5c12e042b01798d7f0dc8bec15543da9d807a33 / DESIGN.md:10 | `object_exists` and `unreachable_objects` changed public behaviour with no `design/` sentence -> the sole-authority rule and §13's same-change requirement were not met -> a reviewer had no living authority to check the behaviour against | introduced_by_feature | docs-contract | 335bb27 | fixed as text at 87c29fc9497fbe3bb3193ea8484cbab449e53cd0: `design/26_design_merge_queue_protocol.md` gains a paragraph after the resume table (added, not reworded; the file is not pinned by `src/export.rs`) stating that every read the engine makes to decide those rows is an inspection, that an inspection that fails propagates as an error and is never read as absence, difference, nothing unreachable or unregistered, and that the store is established readable before Git's absence answer is taken | fixed |
| PR128-REVIEW2-REGISTRATION-GITDIR-STATTED-NOT-READ | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:3249 | the registration scan called `symlink_metadata` on each `gitdir` and never read it -> a `gitdir` at mode 000 stats fine, and `git worktree list` exits 0 and omits that worktree (measured on git 2.43), as does an empty `gitdir` -> `record_for` answered `None`, `add_state` `Unregistered` and the classifier `ObjectResidue::None` for a registration this process could not read; the pass-1 test chmod-ed the containing directory, so it missed the case | introduced_by_feature | correctness | 87c29fc | fixed at 55ed9d3673ff4d6c3f549e8eb3811b394e9b8635: `registration_of` reads each `gitdir` with `fs::read` and hands the bytes to `registration_checkout`, which refuses an empty or malformed one, so an unreadable or empty registration is an inspection error naming the file; `a_registration_whose_gitdir_cannot_be_read_is_an_error_and_not_an_absent_one` sets only the `gitdir` to mode 000 as uid 1000 with the bite asserted, executes the measurement (git omits it, exit 0) and asserts the error from `record_for` and the classifier, then empties the file and asserts the refusal; witnessed failing with the read restored to a stat, and with `record_for` trusting the omission | fixed |
| PR128-REVIEW2-STORE-SCAN-CLAIMS-MORE-THAN-IT-READS | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:3004 | the object-store scan read only the directory `--git-path objects` names and ignored `objects/info/alternates` -> in a `git clone --shared`, making the alternate's pack unreadable left the primary scan passing -> `object_exists` answered `false` while the design sentence the same repair added said absence is taken only from a readable store: the guarantee was false wherever a store is shared | introduced_by_feature | correctness | 87c29fc | fixed at 55ed9d3673ff4d6c3f549e8eb3811b394e9b8635 by deletion: the scan and every claim resting on it are gone, `read_only_verify` takes Git's own absence answer and says at the site that an unreadable pack is indistinguishable from a missing object, and the design paragraph no longer claims readability; what establishing it would take is the deferred row below | fixed |
| PR128-REVIEW2-STORE-SCAN-HANGS-ON-A-FIFO | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:3037 | the scan opened every name ending in `.pack` or `.idx` without checking its file type -> a FIFO named `objects/pack/ignored.pack`, which git ignores, blocks `File::open` until a writer appears -> a residue classification, or any `object_exists`, hangs for ever, with no timeout or cancellation anywhere on the path; reproduced by the reviewer | introduced_by_feature | liveness | 87c29fc | fixed at 55ed9d3673ff4d6c3f549e8eb3811b394e9b8635 by deletion: no production path opens a file in the object store, and the classifier's filesystem reads are `symlink_metadata` and `fs::read` on named administrative files, none of which blocks on a FIFO; the deferred row below states that a real readability check must bound every open | fixed |
| PR128-STORE-READABILITY-NOT-ESTABLISHED | P3 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:3004 | git reports an object it cannot read as one it does not have: with a pack unreadable, `rev-parse --verify --quiet <id>` with a peel exits 1 in silence, exactly as for a missing object (measured on git 2.43) -> nothing at this layer distinguishes a storage failure from absence -> `object_exists` answers `false`, and `refuse_absent_source` refuses a dispatch naming a lost candidate, for a repository whose store is unreadable; latent, since a store the engine cannot read fails at its next write anyway | pre_existing | correctness | 7a83e69 | deferred, with the proposal: proving a store readable means reading it the way git does — `objects/info/alternates` followed to each alternate, a file-type check before every open, and a bound on every open so a FIFO cannot block — which belongs to whoever owns the object store in `src/workspace_manager.rs`, not to a residue classifier; this pull request's first attempt at it produced the three rows above. Until then `read_only_verify` documents exactly what its answer means | deferred |
| PR128-REVIEW2-PATHS-READ-AS-UTF8 | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:2868 | `git_dir_of` read the `.git` pointer with `read_to_string`, and `object_directory` and `common_git_dir` converted git's printed path with `String::from_utf8` -> in a valid repository whose path contains byte `0xff`, git writes and accepts that byte in a linked worktree's pointer and prints it back -> the add classifier answered `Io` where it had answered `After`, and every residue element read through the objects directory failed, against §8 in code this pull request rewrote | introduced_by_feature | portability | a17b8c5 | fixed at 55ed9d3673ff4d6c3f549e8eb3811b394e9b8635: all three decode bytes through `decode_path` (made `pub(super)`, the module family's own §8 decoder, which on Unix cannot fail and elsewhere refuses non-UTF-8 by name) after `trim_ascii_space`; `a_repository_whose_path_is_not_utf8_is_still_classified` builds a repository and a linked worktree whose paths carry `0xff`, passing them to git as bytes, and asserts the pointer git wrote is not UTF-8 and that the classifier answers `After`; witnessed failing with the pointer read restored to `read_to_string` | fixed |
| PR128-REVIEW2-SYMLINKED-GIT-DIR-RECLASSIFIED | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:2904 | `git_dir_of` checked the pointer's target with `symlink_metadata(..).is_dir()`, which is false for a symlink to a directory -> an administrative directory moved and symlinked at its old name is one git reads (`git status`, `rev-parse --git-dir` and `worktree list` all work) -> a populated worktree classified `Internal` with `RegisteredUnpopulatedWorktree` observed, a reclassification the body did not disclose | introduced_by_feature | correctness | a17b8c5 | fixed at 55ed9d3673ff4d6c3f549e8eb3811b394e9b8635: the kind check follows the link (`fs::metadata`), and the site says why, and where `symlink_metadata` is kept because refusing a link is the point (`name_present`, the containment walk); `a_git_directory_reached_through_a_symlink_is_a_populated_worktree` performs the reviewer's relocation, executes git's own read through the link, and asserts `After`; witnessed failing with `metadata` restored to `symlink_metadata` | fixed |
| PR128-REVIEW2-ERROR-CONTRACTS-FALSE | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager.rs:2362 | `WorkspaceManager::object_exists` documented a Git error while the store scan made it return `UpstrokeError::Io`, which the pull request's own test asserted, and `classify_object_residue` documented I/O only for residue names while the same scan returned it for object-store paths -> a caller matching on the documented variant misses the one it gets -> §13, in a contract this pull request had just rewritten | introduced_by_feature | docs-contract | 87c29fc | fixed at 55ed9d3673ff4d6c3f549e8eb3811b394e9b8635: with the scan gone `object_exists` returns `UpstrokeError::Git` and only that, and says so together with what its `false` does and does not mean; `classify_object_residue` and `observed_residue_elements` name `Git` (command, directory, status), `Io` (a residue name, a worktree's `.git` pointer, or a file under `worktrees/`) and `Refused`, which is what they now return | fixed |
| PR128-REVIEW2-DESIGN-PARAGRAPH-SPLITS-THE-TABLE | P2 | f161c9e8555c88474f09f89f095737346070d334 / design/26_design_merge_queue_protocol.md:304 | the added paragraph was placed before the resume table's last row -> the blank line terminated the table -> the `task_merged` row rendered as orphaned pipe text, so the change altered the structure of the sole design authority rather than adding to it | introduced_by_feature | docs-contract | 87c29fc | fixed at 55ed9d3673ff4d6c3f549e8eb3811b394e9b8635: the paragraph follows the whole table, the last row is back inside it, and the paragraph is narrowed to what the code does — a failed inspection propagates, absence is only what git answers, and establishing a store readable is the store's own concern; the rendered structure was checked row by row | fixed |
| PR128-COMMONDIR-UNWRITTEN-READ-AS-AN-INSPECTION-FAILURE | P2 | f161c9e8555c88474f09f89f095737346070d334 / src/workspace_manager/residue.rs:464 | `git worktree add` creates the administrative directory and writes `commondir` after, so a kill in between leaves a zero-length one -> git 2.43 then refuses to enumerate any worktree ("failed to read `…/commondir`", exit 128), which since PR #127's `record_for` repair is an error -> `add_state` propagated it and the classifier answered no class for durable state the site registers; the sampling harness counted it unclassified in 2 of 26 box runs, always `Worktree.Add: n=8 none=1 internal=6 after=0 unclassified=1` | fix_regression | correctness | PR127-RECORD-FOR-COLLAPSES-A-GIT-FAILURE-INTO-ABSENCE | fixed at 55ed9d3673ff4d6c3f549e8eb3811b394e9b8635, closing `SWEEP-WORKTREE-012` on PR #131 by reference: `add_state` reads the registration itself before Git is asked to enumerate, and a `commondir` that is zero-length or absent is the interrupted add, so the site answers `Internal` with its element; `an_add_killed_before_commondir_is_written_is_interrupted_add_residue` plants the zero-length file, executes git's refusal, and asserts the class and the element, and the pre-existing `a_failed_worktree_list_is_an_error_not_an_absent_registration` keeps `record_for`'s error pinned with its classifier half updated to the new answer; witnessed failing with the registration read removed from `add_state` | fixed |
| PR128-NON-UTF8-TEST-CANNOT-RUN-ON-MACOS | P2 | f5b75d7a8088c801f7efa599ee1afe04c0ba6eb9 / src/workspace_manager/tests.rs:6810 | the §8 witness built its fixture with a `0xff` byte in the directory name under `#[cfg(unix)]` -> macOS rejects a filename that is not valid UTF-8 at creation, APFS answering `EILSEQ` -> CI's `test (macos-latest)` leg failed at `f5b75d7` ("a repository directory whose name is not UTF-8: Os { code: 92, kind: Uncategorized, message: \"Illegal byte sequence\" }", 1892 passed, 1 failed), a defect in this pull request's own test and not in the code it guards, since every other job was green | introduced_by_feature | portability | 55ed9d3 | fixed at b85d41e74626c5efdec66eb3893ce44ac6a50965: the test is `#[cfg(target_os = "linux")]`, and its doc says macOS refuses such a name at creation and a Windows path is UTF-16, so the state exists on Linux and is exercised by the local suite and CI's `test (ubuntu-latest)` leg; the assertion is unchanged and no runtime skip was added, so the leg that runs it cannot pass vacuously; `RUSTFLAGS="-D warnings" cargo check` for `x86_64-apple-darwin`, `aarch64-apple-darwin` and `x86_64-pc-windows-msvc` is clean at this head | fixed |
