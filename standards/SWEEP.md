...
|---|---|---|---|
| `src/rundir/classify.rs` | `7098956` | 2026-09-04 | The `#107 rundir` family's first file, and the first sweep outside `workspace_manager`. §6: no shared ownership, lock or clone; the one copy is the line's bytes becoming the owned `Vec` the parser reads, stated at the site. §7: two of the `.ok()?` folds are honest absence (a first line that is not UTF-8, and one that is not this header) and five turn an undecidable I/O answer into `Husk`, which is what `startup_census`'s "anything else" gives; they are settled fold by fold in `reviews/FINDINGS.md` §56 with what the census does with each, and one `?` that could not fire is gone. Fixed: the scan's `Interrupted` branch spent no budget, so a source that answers it for ever was scanned for ever under the physical worktree lock, and the retries are now bounded by `INTERRUPTED_RETRIES`; the line's re-read carried the scan's proof, so a source rewritten between the two reads returned bytes with a newline inside them or with no terminator at all, and every property is now re-established on the bytes returned; a short-read clamp removes an indexing panic and a budget underflow; and the module's safety argument, which said a husk is never deleted without a proof that `committed.json` is absent, is corrected -- `NothingBound` reclaims the public half with no such check, and what protects a directory this probe could not read is `unbound_shape`'s listing. Tests: four added to `src/rundir/tests.rs`, seven mutations run on the box and each killed by a named test. Deferred with the file named: the writer-less-fifo `open` race, measured blocking past five seconds and returning in 3.737us with `O_NONBLOCK`, which no spelling available to this module can close because every non-blocking open is a governed primitive and this module takes no `effects/allowlist.toml` row (row 19, as a site-taking non-blocking read-only open); `read_dir_names` answering `[]` for a failed listing, which is the reclaiming answer (rows 19 and 15); the census's inability to tell a husk from a directory it could not read (`src/engine/topology/startup.rs`); and the untyped `run_started_sha256` string with its two producers (`src/events/log.rs`). |
