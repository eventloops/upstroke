...
|---|---|---|---|
| `src/workspace_manager/naming.rs` | `a7b7c98` | 2026-09-03 | First pass `51feba7`, reviewed at `9f83b09` (PR #118, six findings, all fixed in `a7b7c98`). §6: no shared ownership or lock; the one clone call, `slot.clone()` in the first pass's round-trip test, was removed in the repair, and the file has none now. §7: `from_intent_name` has five `?` sites, one per clause of the grammar, plus the round-trip exit; each returns the parser's `None` and is dispositioned in its doc comment in terms of what "not an intent name" and "malformed intent name" mean to the directory walk, which has one action for both. Fixed: `from_intent_name` accepted non-canonical integers (`g03`, `g+3`) and so was not `intent_name`'s inverse; it now round-trips, pinned at the parser and, in `tests.rs`, through `reclaim_intents`. `safe_component` returns `Result` (§5); `validate` reuses `kind()`. Unit tests in the file, including the intent record's persisted schema with each field dropped independently. Deferred to row 9 (the parent): `write_synced` stages an intent as `<stem>.tmp` in the intents directory, and `intents()` refuses that residue after an interrupted write. |
