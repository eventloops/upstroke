...
|---|---|---|---|
| `src/topology/effects/bijection.rs` | `c82d2e6` | 2026-09-04 | The ST-07 bijection check, and the first file of the `#106 effects` family. §6: no shared ownership, no lock and no clone written by this file; the two copies are `entry.resume_action.clone()` and its before-phase counterpart, each a resume action's own text moving into the failure that compares them, stated at the site. §7: no `?`, and the panic surface is empty -- the `.expect()` recovering a hook phase from `EntryPhase::hook_phase()`'s `Option`, and the `#[expect(clippy::expect_used)]` over it, are gone, the required-coordinate list now carrying both spellings of each coordinate so the `Option` is never produced; the duplicate sweep's `entries[..index]` is `entries.iter().take(index)`; measured with `cargo clippy --all-targets --all-features -- -W clippy::unreachable -W clippy::indexing_slicing`, which reports zero sites of either construct in this file. **Fixed:** `ExecutedInFastSequence` sat in the `else` of "does the no-execution record name this sequence", so a site that ran inside an exercised fast sequence the record does not name was reported only as a gap in the record -- the two are now independent questions and `UnwitnessedFastSequence` says what it checked; recovery-proven evidence on a hook phase now answers `MissingEvidence` and leaves records of a class the entry is not about unread; a failing synthetic record names its element and which of construction, recovery and classification failed, for every failing element rather than the first; the expected classification is read from `ResidueClass::classified_as` rather than written in as `ObjectResidue::Internal`. **§5:** `BijectionFailure` carries `EffectSiteId`, `HookPhase` and `EntryPhase` where it carried `String`, and the typed `RegistryError` where it carried that error's text, which retired three assertions that pinned a coordinate by substring -- one of them `phase.starts_with(point.name())`, a prefix test `Written` and `WrittenFull` already collide on. The two `String`s left are a resume action and a fast sequence's name, both a document's own words. `check_bijection` is `#[must_use]`, and one order-enumeration spelling serves hook and residue phases where the residue loop took `observable_orders()[0]`. The module doc now states what an empty answer means: no I/O and so no inspection failure to misread, but two narrowings -- `inventory`, which was documented, and `host`, which was not and is the one that produced a false success for `Process.Spawn` once already. Eight mutations run on the box, six killed by a named test and **two recorded as unwitnessed with the reason**: reading `classified_as` and unifying the order enumeration are both unobservable at this head, there being one `ResidueClass` and at most one observable order per site. The parent re-export was verified rather than trusted: two public items, both re-exported. Deferred, each naming its file: the frozen sample count N is the registry's own number and "frozen" is a property across runs no single-document check can hold (`SWEEP-BIJECTION-005`, beyond reach -- escalate rather than re-defer if a pass labels it); ST-07, `completeness_rule` and `structure` have no section in `design/` at all, so the family's authority is a packet outside the repository (`SWEEP-BIJECTION-006`, row 28); and `RegistryError` still carries its site and phase as `String`, so the §5 answer stops one file short (`SWEEP-BIJECTION-007`, row 23). |
