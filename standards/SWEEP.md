...
| `src/workspace_manager/fixture.rs` | `2b4551c` | 2026-09-04 | Test support, so §7's panic policy permits `expect` and `panic!` here and the sweep is §5, §12 and the correctness of what the fixture builds; §6 found no shared ownership, lock or clone. **Every Git command the module runs is now built in one place**, mirroring `WorkspaceManager::command`, and Git's repository-locating environment is removed there: measured on git 2.43.0, with `GIT_DIR` set `git -C <fresh> init -q -b main --object-format=sha1` creates no repository at `<fresh>` at all — it re-initialises the repository `GIT_DIR` names, exits 0, and every command after it reads and *commits into* that repository, which is what a suite run from a Git hook, `git rebase --exec` or `git bisect run` would have done. The same builder pins the hooks path (an ambient `core.hooksPath`, or a hook `init.templateDir` copied into `.git/hooks`, ran during the fixture's own `checkout` and `commit` and wrote into the tree every later assertion reads), the fsmonitor, the attributes file (whose `text`/`eol` attribute overrides the `core.autocrlf=false` the file already pinned), the excludes file, signing, and the six identity variables (environment identity overrides repository config, so the `user.name` and `user.email` the fixture wrote were not what decided; the commits are now the same object ids on every machine). The module doc states every pin with its mechanism and the three inputs inherited on purpose — the `git` binary, the system config file, and object replacement. Also fixed: `scratch` discarded its pre-clean's error and created with `create_dir_all`, so a tree a previous run left at its predictable name — and a kill child always leaves one — became this run's fixture in silence; `KillableGitChild` had no `Drop` and `std::process::Child` neither kills nor reaps, so a panicking sampler left a real `git` running in a worktree the harness then removed; `exited` folded a failed `try_wait` into "still running"; `git` returned a lossy string used as an identity; `adopt` read `main~1` for a field documented as the first commit; and `run_kill_child`, now `run_child_test`, returned only an exit status while a filter that matches nothing runs nothing and exits 0, so a renamed child read as a completed injection. §5: an `ObjectFormat` enum for the string `git init --object-format` takes, `Fixture::task` through the manager's own `Slot::validate` and `scratch` through `safe_component`, `SCRATCH` private, `scratch_at` and `with_execution_root` separated out. Fourteen tests and three `#[ignore]`d children on Linux (two of the fourteen `#[cfg(unix)]`) where the file had none, each witnessed under a stated mutation; the unwitnessed claims — `GIT_ATTR_NOSYSTEM`, and a failing `try_wait` — are named in the pull request body rather than left to read as covered. One change outside the file: the rename's import and call in `src/engine/topology/scaffold.rs`, signature unchanged. `effects/allowlist.toml`'s row is re-measured (23 disallowed-method sites, not 15) and its containment clause now says which half is enforced. `reviews/FINDINGS.md` §54 has every row. Deferred to row 11 (the parent): `read_only_git` and `WorkspaceManager::command` inherit the environment this file now strips, so `derive` under an ambient `GIT_DIR` takes its `repo_key` from another repository's common git dir; `write_file` and `create_dir` take any path, so the allowlist's containment clause is caller discipline; and the fixture's three commits are `String` where the tree now has `ObjectId` |
