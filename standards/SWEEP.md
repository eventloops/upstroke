...
| `src/config/parse.rs` | `1755431` | 2026-09-04 | The six section readers of `upstroke.toml`, swept under the owner's amendment 7. §6: no shared ownership, lock or clone; the one copy per refusal is the file's path into the error value, made in `config_error`, the helper that replaced three closures and four inline constructions of the same error. §7: every `?` is same-type propagation after a `map_err` that names the section, and every `unwrap_or`, `map_or` and `unwrap_or_else` folds an *absent* key, never a failed reading -- a written key that cannot be read is refused by the section's `try_into` first, and the module doc says so; the two panic-surface lints report no site in the file (the command is in PR #150). **Fixed:** an unknown key in `[interaction]` and in a `[[gates]]` entry loaded without a word -- `ask_befor = { … }` was a deleted spend approval, `mod = "never"` a CI run that stops to ask, `timeout_sec = 3600` a gate killed at the default -- and `RawInteraction` and `RawGate` now refuse it (the two lines this sweep changes in the parent, its immediate call site); two `[[gates]]` entries with one name were accepted although the name keys the attempt's log file and the failure report; a sequential resume with `max_parallel = 3` alone announced two companion values the file does not contain; a `[runner]` refusal quoted a `kind = "host"` the operator never wrote, and named only the first unknown key. §12: nine tests in the file on `toml::Value` fixtures, each witnessed against a named mutation, two of them pinning refusals the parent's suite left unpinned (an `ask_before` threshold that is negative or not finite; a `[budgets]` ceiling that is not finite); the parent's tests that pin this file's other refusals were each witnessed against a mutation as well. §13: a `# Errors` contract on every reader, a refusal names section, key and value, and no byte offset is carried (spans are gone at `toml::Value`; a `toml::Spanned` reading is row 54's), and `design/17` now states what the repo-level file refuses. Deferred, each with a file: a misspelled section header is dropped whole by `RawRepoConfig` (`SWEEP-CONFIG-PARSE-007`, P2, rows 53 and 54); `RawStrategy` drops unknown keys (`SWEEP-CONFIG-PARSE-008`); a relative mount `source` or `target` is accepted while mounts are inert (`SWEEP-CONFIG-PARSE-009`); credential-volume agent ids are not checked against the adapter registry (`SWEEP-CONFIG-PARSE-010`); `gates::run_all` folds distinct names into one log file (`SWEEP-CONFIG-PARSE-011`); an unknown `shell` warns and degrades, kept as the product decision the parent's suite pins (`SWEEP-CONFIG-PARSE-012`, accepted-risk). Refusals stay `UpstrokeError::Config` strings, so a test pins a substring; the typed refusal is row 54's flattening decision. |
