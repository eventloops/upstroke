...
| `src/agent/proc/ambient.rs` | `4ae4b38` | 2026-09-04 | The first file of the `#117 agent/proc` family, swept under amendment 7 (PR #147). §6: no shared ownership, lock or clone -- none written, and the file holds none; the `OnceLock` and the `Mutex` it fronts are the parent's (`windows_job::AMBIENT`, `termination::CONTAINER_SCOPE`). §7: two `?` in `join_ambient_job_with`, each after a `map_err` into a `Refused` naming the operation, kept as deliberate and spelt out in the `# Errors`; no panic, index, slice or `unreachable!`. Correctness: no defect in the file. What it reads from its environment is nothing, stated in the module doc; the one environment read reachable from it is `termination`'s `PATH` resolution of a bare reaper program on Unix, refused rather than defaulted, made at arming and again per reaper start (`SWEEP-AMBIENT-010`, row 51, where the second is folded by `.ok()`). Fixed as text: the `INV-18`, `crash_reconstruction` and `decisions.*` citations are declared the retired packet's wording and not authority, since `design/15` describes only the private per-invocation Job Object and no public document defines `INV-18`, which the operator-facing refusal names (`SWEEP-AMBIENT-002`, the owner's); both joins' `# Errors` name all three refusals, `set_container_reclaim_scope`'s names the `PATH` and non-UTF-8 refusals, and the two `Option` oracles say a failed kernel query is also `None` (the parent's `ambient_contains` doc corrected with them). Shape: `set_container_reclaim_scope` takes the two-`cfg`-function form its sibling has, `_scope` named rather than `let _ = scope`; `poison_ambient_for_tests` is `#[must_use]`. Tests: the Unix contract "the hook is not consulted" had none -- `the_unix_ambient_join_is_a_no_op_that_consults_no_observer`, witnessed on the box against consult-and-apply and consult-and-discard; every Windows behaviour of the file is pinned by a `#[cfg(windows)]` test named in the PR body's table with the mutation it would catch, none of them run here. Deferred: the raw `FILETIME` identity in `process_alive` (`SWEEP-AMBIENT-011`, row 51; pid identity is an open owner decision and was not touched). |
