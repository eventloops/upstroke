...
valid_record="$(printf 'TACTUS_FRONTIER_REVIEW: 1\nVERDICT: PASS\nREVIEWED_SHA: %s' "$reviewed_sha")"
...
expect_fail "stale structured SHA" "$(printf 'TACTUS_FRONTIER_REVIEW: 1\nVERDICT: PASS\nREVIEWED_SHA: %s' "$stale_sha")"
expect_fail "mixed PASS and FAIL" "$(printf 'TACTUS_FRONTIER_REVIEW: 1\nVERDICT: FAIL\nVERDICT: PASS\nREVIEWED_SHA: %s' "$reviewed_sha")"
expect_fail "duplicate verdict" "$(printf 'TACTUS_FRONTIER_REVIEW: 1\nVERDICT: PASS\nVERDICT: PASS\nREVIEWED_SHA: %s' "$reviewed_sha")"
expect_fail "multiple structured SHAs" "$(printf 'TACTUS_FRONTIER_REVIEW: 1\nVERDICT: PASS\nREVIEWED_SHA: %s\nREVIEWED_SHA: %s' "$stale_sha" "$reviewed_sha")"
expect_fail "current SHA outside stale record" "$(printf 'TACTUS_FRONTIER_REVIEW: 1\nVERDICT: PASS\nREVIEWED_SHA: %s\nCURRENT_SHA: %s' "$stale_sha" "$reviewed_sha")"
expect_fail "quoted markers" "$(printf '> TACTUS_FRONTIER_REVIEW: 1\n> VERDICT: PASS\n> REVIEWED_SHA: %s' "$reviewed_sha")"
