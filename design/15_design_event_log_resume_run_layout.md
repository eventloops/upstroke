...
**When the filesystem declines to answer.** An incomplete listing cannot establish that a run directory is empty. Both ownership proof and public-half removal require a complete listing before authorizing deletion. Any listing error, including `NotFound`, retains the husk or refuses its removal before touching its contents.

Enumeration of `.upstroke/runs` distinguishes a missing root from an incomplete walk. Only a root that both directory-open and metadata probes report absent means no runs. Other open errors, per-entry errors, and entry-metadata errors propagate with their operation and path. Worktree acquisition refuses on these failures, and startup fails before applying a partial census. Run readers report the error; capacity and validation report a warning when optional run observations cannot be loaded. Directory names remain exact filesystem names when probing cleanup leases, including names that are not valid UTF-8. The run-id view skips names that cannot be represented as UTF-8 without reconstructing paths from a lossy rendering.

Only `NotFound` proves a recorded private target or `committed.json` absent; other stat failures retain the husk. Before a missing target authorizes reclaim, its locator must match this run's child of the authorized runs directory. The accepted spellings are the raw root joined with `runs/<run-id>`, the canonical root joined with `runs/<run-id>` (the creator's spelling), and the canonical runs directory joined with `<run-id>`. Comparisons use paths, not rendered strings. The owner's public path must equal the canonical public path. Failed canonicalization establishes no identity and retains the husk with its path and error.

The creator's abort path uses the same ownership proof and reports removal failures. Private removal must succeed before public removal is attempted. If public removal fails after that, the report records the completed private removal and the failed public cleanup; it does not claim the public directory is gone. A failed public removal with no private half bound reports no completed removal. Removal can fail after partial or completed effects, so its error alone does not establish what public residue remains. An unlistable public directory is refused before its marker or other contents are touched.

The marker, owner record, and `run_started` still carry paths as `String`. A private root or public path that is not valid UTF-8 cannot round-trip through those records and is retained and reported rather than reclaimed. Recording these paths losslessly is a separate design change.
